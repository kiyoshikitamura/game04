-- R09: GAME04 dev only. No historical backfill and no gameplay state mutation.
create table if not exists public.game04_state_restore_observations (
 user_id uuid not null references public.users(id), request_id uuid not null,
 observed_version bigint not null check(observed_version>=0), observed_at timestamptz not null default now(),
 primary key(user_id,request_id)
);
alter table public.game04_state_restore_observations enable row level security;
revoke all on public.game04_state_restore_observations from public,anon,authenticated;
grant select,insert on public.game04_state_restore_observations to service_role;
create index if not exists game04_state_restore_at_idx on public.game04_state_restore_observations(observed_at);
...
revoke all on function public.game04_observe_state_restore(uuid,uuid,bigint) from public,anon,authenticated;
grant execute on function public.game04_observe_state_restore(uuid,uuid,bigint) to service_role;
