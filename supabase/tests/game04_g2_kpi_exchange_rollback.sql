...
begin
 ...
 if (select result#>>'{receipt,gameplayMeasurement,action}' from public.game04_requests where user_id=uid and request_id=rid) is distinct from 'shop_exchange' then raise exception 'KPI_RECEIPT_MISSING';end if;
 if (select count(*) from public.game04_requests where user_id=uid and request_id=rid)<>1 then raise exception 'KPI_DUPLICATE';end if;
