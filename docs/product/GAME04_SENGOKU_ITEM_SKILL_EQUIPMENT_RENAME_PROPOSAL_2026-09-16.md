...
- 対象件数:
  ...
  - Equipment: 170件
- 画像制作: **後工程へ延期**
- 本文書は名称候補の保存を目的とし、DB変更・Migration・実装変更を行うものではない。

## 2. 変更禁止事項

GAME03から継承する以下は変更しない。
...
GAME04では、原則として**表示名称と画像のみを戦国世界観へ置換する**。

専用Skill / 専用Equipmentに残る `char_*` はGAME03の現行Authorityとの対応確認用であり、GAME04の60武将への最終割当は別工程とする。

## 3. 命名方針

1. 現代ストリート、銃器、サイバー、英語中心の語彙を戦国世界観へ置換する。
2. 名称から攻撃・防御・回復・速度・状態異常等の性格が可能な限り推測できるようにする。
3. 武芸、兵法、忍術、薬術、甲冑、刀槍弓、火縄銃、陣太鼓、軍配、護符、根付等の語彙を優先する。
4. 汎用装備に特定の実在武将固有名を安易に使用しない。
5. 「天下布武」等の著名な固有語は、将来の該当武将専用要素へ温存する。
6. 画像モチーフは後工程で別途FIXする。
...
- `SKILL_041` の「天下布武」は汎用Skillから外し、GAME04案では「覇道の大号令」とする。
- 「天下布武」は織田信長等の専用要素として使用できる余地を残す。
- `SKILL_051`〜`070` の専用Skillは、性能を維持したままGAME04武将割当時に名称の最終調整を許容する。

---

# 6. Equipment 170件

Equipmentは `WEAPON / BODY / HEAD / LEGS / ACCESSORY` のカテゴリ、ID、Rarity、基礎値、固定効果、専用条件を維持する。
今回変更するのは表示名称案のみ。
