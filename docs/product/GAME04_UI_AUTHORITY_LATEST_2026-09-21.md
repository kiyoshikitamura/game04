...
## 共通UIルール正本 v1.3（2026-09-22）

[共通UIルール正本 v1.3](GAME04_共通UIルール正本.md)を参照。2026-09-22にユーザーから提示された原本全文（第0〜19節）を収録した。承認済み範囲では従来の色・文字・サイズ・画面枠の指定より同書を優先し、それ以外の画面構造・機能・挙動は既存の最新正本を維持する。Battle Rule・Balance・Master数値・Economyは変更しない。

第16・17節の実装・確認結果は[実装記録](../qa/GAME04_COMMON_PRESENTATION_V13_2026-09-22.md)で管理する。原本の収録は、全節の実装完了・実機受入を意味しない。
