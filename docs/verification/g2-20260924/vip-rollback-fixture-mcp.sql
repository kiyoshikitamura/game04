...
-- MCP evidence variant: the fixture subtransaction is always rolled back before the final SELECT.
-- No outer ROLLBACK follows SELECT, so connectors can return the result rows.
...
DECLARE
 ...
 n integer; rejected boolean; stage record; delivered_count integer; evidence jsonb;
BEGIN
 BEGIN
  ...
  INSERT INTO vip_rollback_test_results VALUES('overall',true,'Synthetic SQL fixture only. Fixture subtransaction rolls back; P02 not accepted.');
  SELECT jsonb_agg(to_jsonb(r)) INTO evidence FROM vip_rollback_test_results r;
  RAISE EXCEPTION USING ERRCODE='ZX001',MESSAGE='ROLLBACK_FIXTURE_SUCCESS';
 EXCEPTION WHEN OTHERS THEN
  -- Every fixture row and balance update has now rolled back. PL/pgSQL variables survive.
  IF SQLSTATE='ZX001' AND SQLERRM='ROLLBACK_FIXTURE_SUCCESS' THEN
   INSERT INTO vip_rollback_test_results SELECT * FROM jsonb_to_recordset(evidence) AS r(test text,passed boolean,detail text);
  ELSE
