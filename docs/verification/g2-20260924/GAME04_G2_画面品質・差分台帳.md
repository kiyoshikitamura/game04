> 2026-09-25 記録照合更新：現在の残件は [統合受入結果](GAME04_G2_統合受入結果.md) のZ/R/D/H一覧に一本化。以下は各実施時点の証拠・履歴であり、「未確認」「追加すべき」「親検証中」をそのまま現行タスクへ戻さない。表示の全画面再監査は最新ユーザー指示で終了し、今後は実機個別指摘と同原因の回帰のみ。未検証表示を合格へ変更したものではない。

# GAME04 G2 画面品質・差分台帳

## R5 追加承認・反映（2026-09-25）

- 実装保存 `9e9a0a67da69ee34c9297b123bdf4bf7e06e233b`、開発API ACTIVE v25 / hash `e0ce840a22d80c01a921ca3596bef4da3aaf7268c2c1b54ecd378472e644bf68`。DB schema追加差分0。[不変Preview](https://game04-e3dlc96zt-kiyoshi-kitamura.vercel.app)、Deployment `dpl_3zionmCc9W3YpcGMsTag8qiykBLu`。限定認証API/DBの受入結果は[独立確認](../g2-20260925/r5/independent.md)に保存。
- [追加承認正本](../../product/GAME04_G2_APPROVALS_2026-09-25.md)が下段の未採用・判断待ち履歴に優先。R4の成立済み保存・機能結果を維持。

|追跡|今回の変更|限定確認と残件|
|---|---|---|
|R12/Q05/Q06/D05/D06|72名称・既存40＋新32画像、素材10点を採用反映。レア度文字継続、バッジ制作不要|107 unique画像HTTP200/SHA一致、正式72＋旧8の本体一覧画像全loaded、390×568「火走り」詳細を確認。独立試験で792性能不変を確認。[画像HTTP](../g2-20260925/r5/asset-http-evidence.json)／[限定表示](../g2-20260925/r5/browser-limited.md)。実機・全使用箇所の合格へ拡張しない|
|R12/Q05|エリア添付10、侵攻5城×3区分15背景を優先反映|原本・正規化・ID対応を保存。35背景SHA/形式、5城60Lvの3区分、開始Lv再生と旧snapshot fallbackを独立確認。旧エリア3＋7案は履歴。限定モバイル表示は親のbrowser-limited記録へ、全画面監査・実機合格には拡張しない|
|R07/Q06/D01〜D03|日次期限・結果日帰属・DM005侵攻令1枚、新規侵攻実HP減少算入を実装|判断クローズ／限定API・DB受入済み。DM005の2→3勝達成・令+1、同ID再送非加算・新ID再受取/前日拒否、新侵攻表示38756→実HP25830→倍率後38745の共有HP/貢献一致、再送/DB一致、旧開催damage1445の互換を確認。日付境界は注入時刻、端数cap/3勝/段階跨ぎは局所試験であり実接続範囲と区別|
|SBG-01|SSR10背景・対応SSRは採用済み。6細則は未承認|正規化・ID対応まで先行。解放・選択・保存は[一括具体案](../g2-20260925/r5/ssr-contract.md)判断後。G3ガチャ入手受入を分離|
|R05/R06/R08/R09/H|既存未受入|今回の素材採用で障害実表示／性能比較・基準／管理者HTTP／外部接続を合格にしない|

## R4 現在欄（R5に変更のない証拠を継承）（2026-09-25）
