...
`U10_MEASUREMENT_CONTRACT_CANDIDATE.md` に稼働DBの記録契約、現状不足、成功receipt/QA区分/再送排除を含む具体候補を保存。正式GAME04行動イベントは未定義のため未採用の名前を本体へ実装していない。U10必須未完として親判断一覧へ引継ぎ。

公開予定legalページの旧通貨CASHを銭へ6箇所是正（terms/payments/tokusho）。旧ゲームURL/旧ゲーム名なし。問い合わせ窓口・運営者等8項目はGAME04_LEGAL未設定、P04の具体依存として記録。法的契約条件の変更なし。
