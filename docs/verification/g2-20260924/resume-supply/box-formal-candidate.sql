...
declare s public.game04_player_state%rowtype; path text[]; target text; size text; r text; v jsonb; current_qty bigint;
begin
 ...
 if array_length(path,1)>1 and (v#>path[1:array_length(path,1)-1]) is null then raise exception 'INVALID_INVENTORY_STRUCTURE'; end if;
...
begin
 ...
 if p.source_metadata ? 'game04RewardVersion' then
  if p.source_metadata->>'game04RewardVersion'<>'APPROVED_GROWTH_V1_20260921'
