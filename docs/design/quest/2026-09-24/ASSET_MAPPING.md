...
- 出撃準備のカードは、正式フレームの元比率 1060:1484 を保持する。添付モックの約 1:3.8 の縦帯へ枠を変形させず、人物を画面側 CSS で追加クロップしない。5列の幅から算出される人物占有率の差は、第13節を適用した比較上の補正とする。
- 武将詳細は共通カードの外寸を拡大する。枠内部の人物位置・比率は変更せず、Dialog内部スクロールで能力値・スキル・末尾操作へ到達させる。
