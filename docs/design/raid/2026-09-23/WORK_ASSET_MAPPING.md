...
新規17点は2026-09-23のユーザー指示「採用しますが、背景のみ今後も素材追加の可能性があります。」により**一括採用済み**。背景は現素材を採用し、今後の追加を可能とする。追加背景まで自動承認するものではなく、城別・エリア別の割当仕様を新設しない。素材採用はレイアウト全体の受入や未実施検証の完了を意味しない。旧SVGは保存する。
...
| 貢献・CTA装飾 | `/ui/raid/v2/panel-sakura-overlay.png` | 右上・右辺の桜枝と薄い和柄、透明オーバーレイ。低opacity合成用 |
| 闇属性 | `/ui/raid/v2/element-dark.png` | 紫の丸形・影炎 |

新背景は採用済みの共通レイド背景であり、Master上の城別・エリア別背景として正式決定したものではない。火属性レイドを炎景色へ変えるなど、素材都合の仕様追加はしない。
...
- 9図柄・6属性・背景の全17点目視検査実施。表示寸法での比較はA担当および統合Previewで実施。
...
追加装飾：承認モック右下パネルの桜配置を参考に専用生成。768×512透過PNG、208KB。図柄周辺に半透明の色味を含むため、表示層で低opacity（目安0.15〜0.3）を用い、文字や数値の背面へ配置する。未承認候補。
