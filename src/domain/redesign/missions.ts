...
export function evaluateMissions(state: RedesignState, config: MissionConfig, now=Date.now()): MissionProjection[] {
  ...
  return config.missions.filter(master => master.enabled).map(master => {
    ...
    if(master.condition.type==='metric'){
      const c=master.condition,p=captureMissionAssets(state).missionProgress!;
      let value=0;
      if(c.daily) {
        ...
        case 'ssr_skill_count':value=Object.keys(p.skill).filter(id=>SKILL_MASTERS.some(m=>m.id===id&&m.rarity==='SSR')).length;break;
