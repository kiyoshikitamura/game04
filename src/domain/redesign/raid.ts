...
export function applyRaidAction(original:RaidRoom,originalState:RedesignState,action:string,payload:RaidActionPayload={},now=Date.now()){
 ...
 if(action==='raid_battle'){
  ...
  if(appliesToSharedHp&&room.hp===0){for(const p of room.participants)if(!p.leftAt&&p.wins>=3&&p.joinedLevel<=room.level)room.rewardGrants.push({id:`defeat:${room.level}:${p.userId}`,userId:p.userId,level:room.level,rewards:master.defeatRewards,claimed:false});
   if(master.type==='unlock'&&room.level<master.maxLevel){room.level++;room.maxHp=Math.round(master.sharedHp*(1+(room.level-1)*0.2));room.hp=room.maxHp;}else room.status='defeated';
  }
  return {room,state};
 }
