...
import { FORMAL_ENCOUNTER_MASTERS } from './raidFormalMaster';
...
export function getRaidMaster(id:string) {const master=[...RAID_MASTERS,...FORMAL_ENCOUNTER_MASTERS].find(m=>m.id===id);if(!master)throw new Error('対象レイドが見つかりません。');return master;}
export function getRoomRaidMaster(room:RaidRoom):RaidMaster { const master=room.raidSnapshot ?? room.territorySnapshot?.raidMaster ?? getRaidMaster(room.masterId);const stage=master.stages?.find(s=>s.level===room.level);return stage?{...master,enemy:stage.enemies.find(e=>e.boss)??stage.enemies[0],enemies:stage.enemies,defeatRewards:stage.defeatRewards}:master; }
...
export function raidEnemy(master:RaidMaster,level:number):EnemyUnit {
 ...
 if(master.masterVersion){const enemy=stage?.enemies.find(e=>e.boss)??stage?.enemies[0]??master.enemy;return {...structuredClone(enemy),initialSp:enemy.stats.sp};}
...
export function applyRaidAction(original:RaidRoom,originalState:RedesignState,action:string,payload:RaidActionPayload={},now=Date.now(),acquisitionMaster?:AcquisitionMaster){
 ...
 if(action==='raid_claim') {for(const g of room.rewardGrants)if(g.userId===state.userId&&!g.claimed){g.rewards.forEach((r,i)=>Object.assign(state,grantReward(state,r,`${room.id}:${g.id}:${i}`,acquisitionMaster)));g.claimed=true;}return {room,state,rewards};}
 if(action==='raid_refresh')return {room,state,rewards};
 if(action==='raid_battle'&&payload.battleId&&room.settledBattleIds.includes(payload.battleId))return {room,state,rewards};
 if(action==='encounter_ignore'){if(master.type!=='encounter'||room.ownerId!==state.userId||!me||me.attempts>0)throw new Error('この遭遇は無視できません。');me.leftAt=new Date(now).toISOString();room.status='expired';return {room,state,rewards};}
 ...
 if(action==='raid_join'){
  if(!me){if(room.participants.filter(p=>!p.leftAt).length>=master.maxParticipants)throw new Error('参加人数が上限に達しています。');me={userId:state.userId,name:payload.name||'参戦者',wins:0,attempts:0,totalDamage:0,joinedLevel:room.level};room.participants.push(me);}return {room,state,rewards};
 }
 if(!me)throw new Error('先にレイドへ参加してください。');
 if(action==='raid_leave'){if(room.ownerId===state.userId)throw new Error('主催者は退出できません。');me.leftAt=new Date(now).toISOString();return {room,state,rewards};}
 if(action==='raid_rescue'){
  ...
  if(room.rescueCount>=3)throw new Error('救援依頼の残り回数がありません。');room.rescueCount++;return {room,state,rewards};
 }
 if(action==='raid_battle'){
  ...
  if(appliesToSharedHp&&room.hp===0){for(const p of room.participants)if(!p.leftAt&&p.wins>=3&&p.joinedLevel<=room.level)room.rewardGrants.push({id:`defeat:${room.level}:${p.userId}`,userId:p.userId,level:room.level,rewards:master.defeatRewards,claimed:false});
   if(master.type==='unlock'&&room.level<master.maxLevel){room.level++;room.maxHp=master.stages?.find(stage=>stage.level===room.level)?.sharedHp??Math.round(master.sharedHp*(1+(room.level-1)*master.sharedHpGrowthPerLevel));room.hp=room.maxHp;}else room.status='defeated';
  }
  return {room,state,rewards};
 }
 throw new Error('対応していないレイド操作です。');
}


/** One independent battle; immutable formal stages retain their complete formations. */
export function raidEnemies(master:RaidMaster,level:number):EnemyUnit[]{return structuredClone(master.stages?.find(stage=>stage.level===level)?.enemies??master.enemies??[raidEnemy(master,level)]);}
