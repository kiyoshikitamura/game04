...
import type { BattleRules, BattleUnit, CharacterMaster, Element, EquipmentMaster, EquipmentSlot, Rarity, RedesignState, SkillMaster, Stats } from './types';
...
export const BATTLE_RULES: BattleRules = { defenseFactor:0.45, advantageMultiplier:1.5, disadvantageMultiplier:0.75, spRecoveryDivisor:120, burstLukDivisor:20, enemySpRecoveryDivisor:30, maxPlayerActions:300, initialSpRatio:0 };
...
export const SKILL_MASTERS: SkillMaster[] = oldSkills.skills.filter(s=>!s.exclusive_character_id).map((s,i)=> {
...
export function buildBattleParty(state:RedesignState):BattleUnit[] {return state.deck.map(member=> {
