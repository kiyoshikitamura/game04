...
export const QUEST_AREAS: QuestArea[] = AREAS.map(([id, name, , description], area) => ({id,index:area+1,name,description,image:`/bg/sengoku/${area % 2 ? 'castle-town' : 'castle-approach'}.jpg`,stages:FORMAL_QUEST_STAGES.filter(stage=>stage.areaId===id)}));
