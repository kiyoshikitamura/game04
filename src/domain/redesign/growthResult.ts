...
export function getGrowthResult(before: RedesignState, after: RedesignState, action: string, payload: Record<string, unknown>): GrowthResult {
  ...
  const add = (label: string, a: string | number, b: string | number) => { if (a !== b) rows.push({ label, before: a, after: b }); };
  ...
  if (ea && eb) {
    add('Lv', ea.level, eb.level); add('LB', ea.lb, eb.lb); add('Lv上限', getEquipmentLevelCap(ea.lb), getEquipmentLevelCap(eb.lb));
  ...
  return { title: titles[action] ?? '操作結果', rows, changed: rows.length > 0 };
}
