...
export interface Reward { kind: 'cash' | 'character_material' | 'skill_material' | 'equipment_material' | 'equipment_lb' | 'soul' | 'equipment' | 'unlock_item'; amount: number; id?: string; chance?: number; }
...
export interface RedesignState { legacyImportedIds?: string[]; homeCharacterId?: string; homeBackgroundId?: string; userId: string; souls: Record<string, number>; version: number; cash: number; diamonds: number; energy: number; energyMax: number; characters: OwnedCharacter[]; skills: OwnedSkill[]; equipment: OwnedEquipment[]; deck: DeckMember[]; materials: { character: number; skill: number; equipment: number; equipmentLb: number; unlock: number }; clearedStages: string[]; vipExpiresAt: string | null; }
export interface RaidMaster { id: string; name: string; type: 'encounter' | 'unlock'; enemy: EnemyUnit; energyCost: number; durationMinutes: number; maxParticipants: number; maxLevel: number; checkpoints: number[]; victoryMultiplier: number; sharedHp: number; participationRewards: Reward[]; defeatRewards: Reward[]; }
export interface RaidParticipant { userId: string; name: string; wins: number; attempts: number; totalDamage: number; joinedLevel: number; checkpoint: number; leftAt?: string; lastResult?: string; }
export interface RaidRoom { id: string; masterId: string; ownerId: string; level: number; hp: number; maxHp: number; expiresAt: string; createdAt: string; status: 'active' | 'defeated' | 'expired'; rescueCount: number; rescueWindowStartedAt: string; participants: RaidParticipant[]; settledBattleIds: string[]; rewardGrants: { id: string; userId: string; level: number; rewards: Reward[]; claimed: boolean }[]; }
