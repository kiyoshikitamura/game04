...
export type TargetRule = 'first' | 'lowest_hp' | 'highest_hp' | 'random' | 'all_enemies' | 'lowest_ally' | 'all_allies' | 'self' | 'dead_ally' | 'lowest_hp_ratio' | 'highest_hp_ratio';
export type SkillCondition = { type: 'always' | 'hp_below' | 'ally_hp_below' | 'every_n_actions' | 'enemy_count' | 'ally_dead'; value?: number };
export type SkillEffect = { type: 'damage' | 'heal' | 'atk_up' | 'def_up' | 'atk_down' | 'def_down' | 'poison' | 'revive' | 'sp' | 'stun'; target?: TargetRule | 'selected'; chance?: number; healingFormula?: 'caster_atk_percent' | 'target_max_hp_percent'; displayHits?: number; power: number; duration?: number; carryAcrossWaves?: boolean };
export interface SkillMaster { unsupportedReason?: string; id: string; name: string; image: string; rarity: Rarity; element: Element; spCost: number; condition: SkillCondition; target: TargetRule; effects: SkillEffect[]; description: string }
export interface Passive { condition?: SkillCondition; level?: number; id: string; name: string; stat: keyof Stats; percent: number; target: 'party' | 'self'; }
...
export interface BattleRules { version?: string; defenseFactor: number; advantageMultiplier: number; disadvantageMultiplier: number; spRecoveryDivisor: number; burstLukDivisor: number; enemySpRecoveryDivisor: number; maxPlayerActions: number; initialSpRatio: number; }
