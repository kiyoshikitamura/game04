...
export interface BattleRules { /** Absent on saved inputs: retain half-SP/basic fallback. */ burstPolicy?: 'attack-free-v1-20260926'; /** Omitted on saved legacy inputs. */ inputVersion?: 'wave-sp-v1-20260921'; balanceV2?: BalanceV2Config; version?: string; defenseFactor: number; advantageMultiplier: number; disadvantageMultiplier: number; spRecoveryDivisor: number; burstLukDivisor: number; enemySpRecoveryDivisor: number; maxPlayerActions: number; initialSpRatio: number; }
export interface RaidBattleStartSnapshot { roomId: string; level: number; hp: number; maxHp: number; }
export interface BattleInput { earlyQuestAssist?: {version:'area1-assist-v1-20260926';stageId:string;actionGainMultiplier:2;burstChance:0.8}; raidDamagePolicy?: 'actual-hp-v1-20260925'; raidStartSnapshot?: RaidBattleStartSnapshot; seed: number; party: BattleUnit[]; waves: EnemyUnit[][]; rules: BattleRules; }
export interface Reward { kind: 'free_diamonds' | 'ticket' | 'character_exp_item' | 'equipment_exp_item' | 'generic_soul' | 'soul_selector' | 'character' | 'skill' | 'cash' | 'character_material' | 'skill_material' | 'equipment_material' | 'equipment_lb' | 'soul' | 'equipment' | 'unlock_item'; amount: number; id?: string; chance?: number; }
