...
export interface RedesignState { energyDrinks?: number; territoryUnlockGrantedVersion?: string; questAttempts?: Record<string, number>; questClearCounts?: Record<string, number>; questTicketGrants?: Record<string, number>; questProgressVersion?: string; dailyNormalGachaDate?: string; growthInventory?: GrowthInventory; playerProgress?: {version: string; level: number; exp: number; status: "active" | "migration_pending"}; territoryItems?: Record<string, number>; claimedMissionIds?: string[]; legacyImportedIds?: string[]; homeCharacterId?: string; homeBackgroundId?: string; userId: string; souls: Record<string, number>; version: number; cash: number; diamonds: number; energy: number; energyMax: number; characters: OwnedCharacter[]; skills: OwnedSkill[]; equipment: OwnedEquipment[]; deck: DeckMember[]; materials: { character: number; skill: number; equipment: number; equipmentLb: number; unlock: number }; clearedStages: string[]; vipExpiresAt: string | null; }
