import { getThemedMasterName } from "../../../theme/masters";
...
export const CANONICAL_CHARACTERS: readonly CanonicalCharacter[] = characterSource.characters.map((character) => ({
  ...
  name: getThemedCharacterName(character.character_id, character.name),
...
export const CANONICAL_SKILLS = skillSource.skills.map((skill) => ({ ...skill, name: getThemedMasterName(skill.skill_id, skill.name) }));
export const CANONICAL_EQUIPMENTS = equipmentSource.equipments.map((equipment) => ({ ...equipment, display_name: getThemedMasterName(equipment.equipment_id, equipment.display_name) })) as readonly CanonicalEquipment[];
