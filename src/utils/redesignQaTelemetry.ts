...
export type QaMetric = {
  kind: 'request' | 'image-group'; scope: string; count?: number;
...
export function emitQaTiming(metric: QaMetric) {
  ...
  const message: QaTimingMessage = {
    ...
    metric: { kind: metric.kind, scope: (metric.kind === 'request' ? requestScopes : new Set(['home','growth','quest'])).has(metric.scope) ? metric.scope : 'other', ...(metric.count === undefined ? {} : { count: metric.count }), startedAt: metric.startedAt, settledAt: metric.settledAt, durationMs: metric.durationMs, outcome: metric.outcome },
...
export function isQaTimingMessage(value: unknown): value is QaTimingMessage {
  ...
  return v.type === 'game04-qa-timing' && Number.isFinite(v.timeOrigin) && !!m
    && ['request','image-group'].includes(m.kind) && typeof m.scope === 'string' && /^[a-z_]{1,48}$/.test(m.scope)
...
export function appendQaTiming(current: QaTimingMessage[], message: QaTimingMessage) {
  return [...(current.length && current[0].timeOrigin !== message.timeOrigin ? [] : current).slice(-99), message];
}
