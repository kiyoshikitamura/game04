...
export default function Game04AccountPage() {
  ...
  return <main style={{ height: '100dvh', overflowY: 'auto', background: '#16130f', color: '#f5eddf', padding: '24px 20px', boxSizing: 'border-box' }}>
    <section style={{ maxWidth: 440, margin: '0 auto', display: 'grid', gap: 16 }}>
      ...
      {(!binding?.linked || loginMode) && <>
        ...
        {user && !loginMode ? <><button style={{ minHeight: 44, padding: '10px 14px' }} disabled={busy || !binding} onClick={() => void run(connectEmail)}>{binding?.needsPassword ? 'パスワードを設定して連携完了' : 'メールアドレスを連携'}</button><button style={{ minHeight: 44, padding: '10px 14px' }} disabled={busy || !user.is_anonymous || !binding} onClick={() => void run(connectGoogle)}>Googleを連携</button></> : <>
          ...
          <button style={{ minHeight: 44, padding: '10px 14px' }} disabled={busy} onClick={() => void run(async () => { localStorage.removeItem(GAME04_AUTH_INTENT); const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${location.origin}/auth/game04/callback`, queryParams: { prompt: 'select_account' } } }); if (error) throw error; })}>Googleでログイン</button>
        </>}
      </>}
      ...
      {user?.is_anonymous && !loginMode && <button style={{ minHeight: 44, padding: '10px 14px' }} disabled={busy} onClick={() => void run(async () => {
        ...
        localStorage.removeItem(GAME04_AUTH_INTENT); setCanRestoreGuest(true); setLoginMode(true);
        setMessage('連携済みのアカウントへログインします。現在の未連携データは統合・削除しません。このタブでは「元の未連携データへ戻る」で復帰できます。');
      })}>連携済みアカウントへログイン</button>}
      {canRestoreGuest && <button style={{ minHeight: 44, padding: '10px 14px' }} disabled={busy} onClick={() => void run(async () => {
