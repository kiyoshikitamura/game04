...
export default function Game04AccountPage() {
  ...
  return <main className="g4-account"><section>
      ...
      {(!binding?.linked || loginMode) && <>
        {user&&!loginMode&&!binding?.needsPassword?<><ActionButton className="g4-account-google" disabled={busy||!user.is_anonymous||!binding} onClick={()=>void run(connectGoogle)}><b aria-hidden="true">G</b>現在のデータをGoogleに連携</ActionButton><p className="g4-account-note">進捗・所持品をそのまま引き継ぎます。</p></>:!user||loginMode?<ActionButton className="g4-account-google" disabled={busy} onClick={() => void run(async () => { localStorage.removeItem(GAME04_AUTH_INTENT); const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${location.origin}/auth/game04/callback`, queryParams: { prompt: 'select_account' } } }); if (error) throw error; })}><b aria-hidden="true">G</b>Googleでログイン</ActionButton>:null}
        <ActionButton className="g4-account-email" disabled={busy} aria-expanded={emailOpen||binding?.needsPassword} onClick={()=>setEmailOpen(v=>!v)}>{user&&!loginMode?'メールアドレスで連携':'メールアドレスでログイン'} <span aria-hidden="true">⌄</span></ActionButton>
        ...
        {user && !loginMode ? <><ActionButton disabled={busy || !binding} onClick={() => void run(connectEmail)}>{binding?.needsPassword ? 'パスワードを設定して連携完了' : 'メールアドレスを連携'}</ActionButton></> : <>
          <ActionButton disabled={busy} onClick={() => void run(async () => { localStorage.removeItem(GAME04_AUTH_INTENT); const { error } = await supabase.auth.signInWithPassword({ email: email.trim(), password }); if (error) throw new Error('メールアドレスまたはパスワードを確認してください。'); setPassword(''); await game04Binding(true); setLoginMode(false); await refresh(); })}>メールでログイン</ActionButton>

        </>}
        </div>}
      </>}
      {user && !user.is_anonymous && !binding?.linked && binding?.method && !binding.needsPassword && <ActionButton disabled={busy} onClick={() => void run(async () => { await game04Binding(true); localStorage.removeItem(GAME04_AUTH_INTENT); await refresh(); })}>連携の確定を再試行</ActionButton>}
      {user?.is_anonymous && !loginMode && <ActionButton disabled={busy} onClick={() => void run(async () => {
      ...
      })}>連携済みアカウントへログイン</ActionButton>}
      {canRestoreGuest && <ActionButton disabled={busy} onClick={()=>void run(async()=>{await restoreGuest();setPassword('');setEmailOpen(false);if(loginMode)returnToGame04();})}>{loginMode?'取消して元のゲームへ戻る':'元の未連携データへ戻る'}</ActionButton>}
      <ActionButton disabled={busy} onClick={() => void run(refresh)}>状態を再確認</ActionButton>
      {binding?.linked && <ActionButton disabled={busy} onClick={() => void run(async () => { const { error } = await supabase.auth.signOut(); if (error) throw error; localStorage.removeItem(GAME04_AUTH_INTENT); await refresh(); })}>ログアウト</ActionButton>}
      <p style={{ fontSize: 14 }}>別端末では、連携した同じGoogleアカウントまたはメールアドレスとパスワードでログインしてください。未連携のデータはブラウザを変えると引き継げません。</p>
      <ActionButton disabled={busy} onClick={()=>void run(async()=>{if(loginMode&&canRestoreGuest)await restoreGuest();returnToGame04();})}>ゲームへ戻る</ActionButton>
    </section>
  </main>;
}
