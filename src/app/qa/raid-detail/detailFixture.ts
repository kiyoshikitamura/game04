...
export function createDetailFixture(scenario:DetailScenario,now:number){
 ...
 const people:readonly RaidParticipantDto[]=Array.from({length:scenario==='many'||scenario==='long-name'?20:3},(_,i)=>({roomId:room.roomId,player:{userId:`qa-person-${i}`,name:scenario==='long-name'?`確認用の非常に長い参加者名ABCDEFGHIJKLMN_${i}`:`確認用参加者${i+1}`,leaderIconUrl:known(RAID_TOP_ENEMIES[i%RAID_TOP_ENEMIES.length].leaderImageUrl)},currentGuild:scenario==='unknown'?unknown:known(scenario==='no-guild'?null:{guildId:'qa-guild',name:scenario==='long-name'?'確認用の非常に長いGuild名ABCDEFGHIJKLMN':'確認用Guild'}),battleGuildSnapshot:unknown,finalizedBattles:scenario==='unknown'?unknown:known(i),rawDamage:scenario==='unknown'?unknown:known(i*135001),appliedDamage:scenario==='unknown'?unknown:known(i*125000)}));
 ...
 const display:RaidRoomDisplay={roomId:room.roomId,ownerGuild:scenario==='unknown'?unknown:known(scenario==='no-guild'?null:{guildId:'qa-guild',name:'確認用Guild'}),membership:role,leaderCharacterIds:Object.fromEntries(people.map((p,i)=>[p.player.userId,RAID_TOP_ENEMIES[i%RAID_TOP_ENEMIES.length].roster[0].id])),clearPlan:plan,rescuePlan:plan};
