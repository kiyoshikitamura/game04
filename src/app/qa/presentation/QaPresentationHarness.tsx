...
const leader = findCharacter("織田信長");
const playerNames = ["織田信長", "豊臣秀吉", "武田信玄", "真田幸村", "上杉謙信"];
const enemyNames = ["本多忠勝", "伊達政宗", "前田慶次", "明智光秀", "徳川家康"];
...
function NameRetryFixture() {
  ...
  if (state === "SUCCESS") return <div className="setup-container is-world-entry" data-name-lifecycle="success"><section className="setup-ageha-presentation" data-entry-state="AGEHA_INTRO"><div className="setup-ageha-character"><CharacterPresentation src="/characters/ageha_transparent_asset.png" alt="豊臣秀吉" variant="dialogue-bust" /></div><div className="setup-ageha-dialogue"><div className="setup-ageha-name">豊臣秀吉</div><span>NEON-Rね。覚えた。よろしく。</span></div><button className="semantic-cta semantic-cta--primary setup-primary-action">次へ</button></section></div>;
  return <div className="setup-container is-registration" data-name-lifecycle={state.toLowerCase()}><div className="setup-box setup-name-dialog"><div className="setup-name-guidance"><strong>豊臣秀吉</strong><span>ここでなんて呼べばいい？</span></div><h2>プレイヤー名</h2><input className="setup-name-input" value={state === "ERROR" ? "NEON" : "NEON-R"} readOnly /><button className="semantic-cta semantic-cta--primary" onClick={() => setState("SUCCESS")}>この名前で始める</button></div>{state === "ERROR" && <div className="modal-overlay"><div className="modal-card border-danger" role="alertdialog"><div className="modal-title text-color-danger">エラー</div><div className="modal-desc">この名前はすでに使用されています。</div><button className="semantic-cta semantic-cta--danger" onClick={() => setState("RETRY")}>閉じる</button></div></div>}</div>;
}
...
function SimpleFixture({ kind }: { kind: QaPresentationScenarioId }) {
  if (kind === "world-introduction") return <div className="tutorial-world" aria-label="World Introduction"><div className="tutorial-world-content"><div className="tutorial-world-shade" /><div className="tutorial-world-ageha"><CharacterPresentation src="/characters/ageha_transparent_asset.png" alt="豊臣秀吉" variant="dialogue-bust" /></div><div className="tutorial-world-dialogue"><strong>豊臣秀吉</strong><TypewriterText text={WORLD_STAGES[1].text} speedMs={22} highlightTerms={[...WORLD_STAGES[1].highlights]} /></div><button className="semantic-cta semantic-cta--primary">次へ</button></div></div>;
  if (kind === "gacha-page") return <section className="qa-card qa-gacha"><span>豊臣秀吉</span><p>まずは10連、引いてみよ。</p><img src="/gacha/bg_gacha_ssr.jpg" alt="ガチャバナー" /><small>無料10連 / SSR1体保証</small><button>無料10連を引く</button></section>;
  if (kind === "gacha-standard-reveal") { const character: any = findCharacter("豊臣秀吉"); return <section className="qa-reveal"><CharacterPresentation src={getCharacterTransparentImg(character.name)} alt={character.jpName} variant="reveal" rarity={character.rarity} attribute={character.alignment} backgroundSrc={getCharacterLocationBackground(character.homeTown)} frameKind="reveal" rarityBadge attributeBadge /><h2>{character.jpName}</h2><small>タップして次へ</small></section>; }
...
function ProductionHomeFixture({ scenario, activityOverride, activityNowOverride }: {
  ...
  const homeLeader: any = findCharacter("豊臣秀吉");
