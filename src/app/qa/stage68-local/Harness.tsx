...
type Fixture={case:string;recipe:string;seed:number;result:BattleResult};
export default function Harness(){
 ...
 useEffect(()=>{if(!fixture)return;let hidden=0;const onVisibility=()=>{if(document.hidden)hidden++;};document.addEventListener('visibilitychange',onVisibility);
  const observer=new MutationObserver(()=>{const battle=document.querySelector<HTMLElement>('[data-playback-frame]');if(!battle)return;const frame=Number(battle.dataset.playbackFrame);if(started.current===null&&battle.dataset.introPaused==='false'&&battle.dataset.playbackPaused==='false')started.current=performance.now();if(started.current!==null&&frame===fixture.result.frames.length-1){setMeasurement({case:fixture.case,seed:fixture.seed,recipe:fixture.recipe,wallMilliseconds:performance.now()-started.current,frames:fixture.result.frames.length,actions:fixture.result.playerActions,reason:fixture.result.reason,hiddenTransitions:hidden,speedLabel:document.querySelector('[aria-label^="再生速度"]')?.getAttribute('aria-label')??'finished',measuredAt:new Date().toISOString()});observer.disconnect();}});
