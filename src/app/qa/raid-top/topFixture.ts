...
export function createTopFixture(scenario: TopScenario, now = Date.now(), areaOffset = 0): RaidTopData {
  ...
  const player = (index: number): RaidPlayerSummary => ({ userId: `qa-person-${index}`, name: scenario === 'long-name' ? '非常に長い名前ABCDEFGHIJKLMN' : `プレイヤー${index + 1}`, leaderIconUrl: known(scenario === 'broken-image' ? '/qa-missing-image.png' : enemyAt(index).leaderImageUrl) });
  const entry = (index: number, rescue: boolean): RaidTopEntry => {
    ...
    return {
      ...
      ownerGuild: scenario === 'unknown' ? unknown : known(scenario === 'no-guild' ? null : { guildId: 'qa-guild', name: scenario === 'long-name' ? 'とても長いギルド名ABCDEFGHIJKLMN' : 'Guild' }),
      ...
      origin: known(index % 2 === 0 ? 'encounter' : 'territory'),
    };
  };
