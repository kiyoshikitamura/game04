'use client';
import ConsumerStateHarness from './ConsumerStateHarness';
...
function fixture(){const s=createInitialState('common-ui-synthetic');s.characters=CHARACTER_MASTERS.map(c=>({id:c.id,level:10,awakening:2}));s.skills=OWNABLE_SKILL_MASTERS.map(c=>({id:c.id,level:2}));s.equipment=EQUIPMENT_MASTERS.map((c,i)=>({instanceId:'fixture-'+i,masterId:c.id,level:10,lb:2}));s.cash=1234567;s.diamonds=10000;s.growthInventory=emptyGrowthInventory();s.growthInventory.expItems.character.small=100;s.growthInventory.expItems.equipment.small=100;s.materials.skill=100;s.materials.equipmentLb=100;const mode=new URLSearchParams(location.search).get('state');if(mode==='empty'){s.characters=[];s.skills=[];s.equipment=[];s.deck=[];}return s;}
...
export default function Harness(){const[mounted,setMounted]=useState(false);useEffect(()=>setMounted(true),[]);return mounted?(new URLSearchParams(location.search).get('view')?.startsWith('consumer-')?<ConsumerStateHarness/>:<Content/>):null;}
