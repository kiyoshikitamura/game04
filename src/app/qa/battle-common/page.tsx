...
const scenarios = { presentation: '承認モック・演出比較', 'six-waves': '最大6Wave・敵開始SP', burst: 'SP・BURST・スキル優先順', interrupt: '割込み・行動不能', waves: 'Wave引継ぎ', limit: '300回上限', legacy: '保存済み旧形式互換' };
export default async function CommonBattleQaPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (params.viewport === '360' || params.viewport === '390') return <main style={{ height: '100dvh', overflow: 'auto', background: '#120d09', color: '#ead3aa', padding: 12 }}><p>スマートフォン幅 {params.viewport}px・仮データ検証</p><iframe title="戦闘ルールのスマートフォン表示" src={`/qa/battle-common?scenario=${scenario}&canvas=1&vip=${params.vip === '0' ? '0' : '1'}`} style={{ display: 'block', width: Number(params.viewport), height: 900, border: '1px solid #ad8150', margin: 'auto' }} /></main>;
  const result = simulateBattle(commonBattleFixture(scenario));
  if (params.canvas === '1') return <main style={{ height: '100dvh', overflow: 'auto', background: '#120d09', color: '#ead3aa' }}><BattleReplayFixture result={result} vipActive={params.vip !== '0'} /></main>;
  return <main style={{ height: '100dvh', overflow: 'auto', background: '#120d09', color: '#ead3aa' }}><div style={{ maxWidth: 500, padding: '12px 10px 40px', margin: 'auto' }}>
    ...
    <BattleReplayFixture result={result} vipActive={params.vip !== '0'} />
  </div></main>;
}
