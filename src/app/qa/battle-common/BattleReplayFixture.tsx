...
export default function BattleReplayFixture({ result, vipActive = true }: { result: BattleResult; vipActive?: boolean }) {
  ...
  return <>
    ...
    <BattleView key={generation} result={result} vipActive={vipActive} initialFrame={frame} initialPaused onComplete={() => { setFrame(0); setGeneration(value => value + 1); }} title="共通戦闘ルール・検証リプレイ" />
  </>;
}
