...
export default function QuestView({ state, party, vipActive, onStart, onOpenDeck, onOpenRaid, onIgnoreEncounter, initialStageId, initialPreparation = false, onBattlePlayingChange, onEarlyAction, navigationBlocked = false }: {
  ...
  return <section className="redesign-quest" style={{ backgroundImage: `url("${area?.image ?? QUEST_AREAS[0].image}")` }}>
    ...
    {settlement ? <div className="rq-summary">
      ...
      {settlement.encounterRaidId ? <><h3>強敵の気配</h3><p>遭遇戦が発生しました。</p><p className="rq-muted">無視すると、この共闘への参加権を失います。</p><ActionButton disabled={busy} onClick={() => onOpenRaid(settlement.encounterRaidId!)}>挑む</ActionButton><ActionButton disabled={busy} onClick={() => void dismissEncounter()}>無視する</ActionButton></> : <>
        <ActionButton onClick={() => { setSettlement(null); setSelected(null); }}>ステージ一覧へ</ActionButton>
        {selected && <ActionButton onClick={() => { setSettlement(null); setModal('prepare'); }}>再挑戦</ActionButton>}
        {settlement.battle.outcome === 'win' && followingStage && <ActionButton onClick={() => { setSettlement(null); setAreaId(followingStage.areaId); openStage(followingStage); }}>次のステージへ</ActionButton>}
        <ActionButton onClick={() => onOpenDeck()}>編成を見直す</ActionButton>
      </>}
    ...
    {selected && modal === 'info' && <div className="redesign-quest-dialog"><CanonicalDialog title={`${selectedLabel} ${formalStageName(selected) ?? selected.name}`} onClose={() => setModal(null)} actions={[{ label: '挑戦', semantic: 'primary', onClick: () => setModal('prepare'), disabled: !encounterAssets.ready || !isQuestStageUnlocked(selected.id, state.clearedStages, state.earlyProgress) }]}>
      {!encounterAssets.ready && <p role={encounterAssets.failed ? "alert" : "status"}>{encounterAssets.failed ? <>画像を読み込めませんでした。<ActionButton onClick={encounterAssets.retry}>再読み込み</ActionButton></> : <LoadingSpinner />}</p>}
      ...
      <div className="rq-info-actions"><ActionButton size="compact" onClick={() => setDetailPanel('hint')}><img src="/ui/sengoku/02-scroll-top.png" alt="" />攻略のヒント</ActionButton><ActionButton size="compact" onClick={() => setDetailPanel('rewards')}><img src="/ui/sengoku/01-gift.png" alt="" />報酬を確認</ActionButton></div>
    </CanonicalDialog></div>}
