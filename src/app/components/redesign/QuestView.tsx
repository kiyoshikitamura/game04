...
function formalStageName(stage: QuestStage) {
  return stage.name.trim() && stage.name.trim() !== stage.description.trim() ? stage.name : null;
}
function formalStageHint(stage: QuestStage) {
  return stage.description.trim() && stage.description.trim() !== stage.name.trim() ? stage.description : null;
}
function selectRepresentativeBoss(wave: QuestStage['waves'][number]) {
  return wave.reduce((best, enemy) => enemy.level > best.level || (enemy.level === best.level && enemy.stats.hp > best.stats.hp) ? enemy : best, wave[0]);
}
export default function QuestView({ state, party, vipActive, onStart, onOpenDeck, onOpenRaid, onIgnoreEncounter, initialStageId, onBattlePlayingChange }: {
  ...
  return <section className="redesign-quest">
    {settlement ? <div className="rq-summary">
      ...
      {area ? <><button className="rq-back" onClick={() => setAreaId(null)}>‹ エリア一覧</button><header className="rq-area-head" style={{ backgroundImage: `linear-gradient(90deg,#100a0888,transparent 76%),url("${area.image}")` }}><h2>{area.name}</h2></header>
        <div className="rq-scroll" aria-label={`${area.name}のステージ一覧`}>{area.stages.map(stage => {
          ...
          return <button className={`rq-stage ${current.id === stage.id ? 'is-current' : ''}`} key={stage.id} disabled={!unlocked} onClick={() => openStage(stage)} style={{ backgroundImage: `linear-gradient(90deg,#302015aa,#14100e88),url("${area.image}")` }}><b className="rq-stage-number">{area.index}-{stage.index}</b><span><strong>{formalStageName(stage) ?? '表示名未定義'}</strong><small>消費行動力 {questEnergyCost(stage,state)}</small></span><small className={`rq-state-label ${cleared ? 'is-cleared' : unlocked ? 'is-current' : 'is-locked'}`}><i aria-hidden="true">{cleared ? '✓' : unlocked ? '◆' : '×'}</i>{cleared ? 'クリア済' : unlocked ? '挑戦可能' : '未解放'}</small></button>;
          ...
          return <button key={entry.id} disabled={!unlocked} className={`rq-area ${entry.id === current.areaId ? 'is-current' : ''}`} style={{ backgroundImage: `linear-gradient(0deg,#0c080690,transparent 72%),url("${entry.image}")` }} onClick={() => setAreaId(entry.id)}><span>第{entry.index}章</span><strong>{entry.name}</strong><span className={`rq-area-status ${cleared ? 'is-cleared' : unlocked ? 'is-current' : 'is-locked'}`}><i aria-hidden="true">{cleared ? '✓' : unlocked ? '◆' : '🔒'}</i>{cleared ? '解放済' : unlocked ? '攻略中' : '未解放'}</span></button>;
        })}</div></>}
    </>}
    {selected && modal === 'info' && <div className="redesign-quest-dialog"><CanonicalDialog title={`${QUEST_AREAS.find(entry => entry.id === selected.areaId)?.index}-${selected.index}`} onClose={() => setModal(null)} actions={[{ label: '戻る', onClick: () => setModal(null) }, { label: '挑戦', semantic: 'primary', onClick: () => setModal('prepare'), disabled: !isQuestStageUnlocked(selected.id, state.clearedStages) }]}>
      <div className="rq-encounter-hero" style={{ backgroundImage: `linear-gradient(180deg,#2b1c2477,#110c0ebb),url("${QUEST_AREAS.find(entry => entry.id === selected.areaId)?.image ?? ''}")` }}><span className="rq-kicker">ステージボス</span>{(() => { const enemy = selectRepresentativeBoss(selected.waves[selected.waves.length - 1]); const subject = bossSubject(enemy); return <div className="rq-bosses"><article key={enemy.id} className="rq-boss rq-boss-single"><div className="rq-boss-stage">{subject ? <BossDisplay subject={subject} compact hideCaption className="rq-boss-display" /> : <div className="rq-boss-missing">正式バトル素材供給待ち</div>}<div className="rq-boss-overlay"><div className="rq-boss-title"><strong>Lv.{enemy.level} {enemy.name}</strong><ElementBadge element={enemy.element} /></div><small>HP {enemy.stats.hp.toLocaleString()} ／ ATK {enemy.stats.atk.toLocaleString()} ／ DEF {enemy.stats.def.toLocaleString()}</small>{enemy.skills.length > 0 ? <div className="rq-enemy-skills">{enemy.skills.map(skill => <span key={skill.id}><img src={skill.image} alt="" />{skill.name}</span>)}</div> : <small>スキル：無し</small>}</div></div></article></div>; })()}</div>
