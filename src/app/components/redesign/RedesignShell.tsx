...
export default function RedesignShell({ state, onAction, children, notifications, activeTab, onNavigate, navigationBusy = false, hideChrome = false, encounterRaid, socialEvents, missions, onRefreshMissions, previewOnly = false }: { state: RedesignState; onAction: HomeAction; children?: React.ReactNode; notifications?: React.ReactNode; activeTab: string; onNavigate: (tab: string) => void; navigationBusy?: boolean; hideChrome?: boolean; encounterRaid?: HomeEncounter | null; socialEvents?: HomeSocialEvent[]; missions?: MissionProjection[]; onRefreshMissions?:()=>Promise<unknown>; previewOnly?: boolean }) {
  ...
  return <div ref={scrollRef} tabIndex={0} role="region" aria-label="ゲーム画面" className={`rd-shell ${hideChrome ? 'rd-shell-battle' : ''}`}>
    ...
    {menu && <Modal title="メニュー" onClose={() => setMenu(false)}><div className="rd-stack"><ActionButton className="rd-button" disabled={navigationBusy} onClick={() => run(() => onNavigate('bag'))}>所持品 ›</ActionButton><ActionButton className="rd-button" onClick={() => run(() => { game.setInboxPanelTab('news'); game.setShowInboxPanel(true); })}>お知らせ{game.unreadNewsCount ? ` (${game.unreadNewsCount})` : ''}</ActionButton><ActionButton className="rd-button" onClick={() => run(() => { game.setInboxPanelTab('presents'); game.setShowInboxPanel(true); })}>プレゼントBOX{game.unreadPresentsCount > 0 && <i className="g4-unread" aria-label="未読あり" />}</ActionButton><ActionButton className="rd-button" onClick={() => run(() => game.setShowSettingsPanel(true))}>設定</ActionButton><ActionButton className="rd-button" onClick={() => run(() => openGame04Account(activeTab))}>アカウント連携</ActionButton></div></Modal>}
    <InboxPanel />
