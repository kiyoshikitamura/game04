...
export default function RedesignShell({ state, onAction, children, notifications, activeTab, onNavigate, hideChrome = false, encounterRaid, socialEvents, missions, previewOnly = false }: { state: RedesignState; onAction: HomeAction; children?: React.ReactNode; notifications?: React.ReactNode; activeTab: string; onNavigate: (tab: string) => void; hideChrome?: boolean; encounterRaid?: HomeEncounter | null; socialEvents?: HomeSocialEvent[]; missions?: MissionProjection[]; previewOnly?: boolean }) {
  ...
  return <div ref={scrollRef} tabIndex={0} role="region" aria-label="ゲーム画面" className={`rd-shell ${hideChrome ? 'rd-shell-battle' : ''}`}>
    {!hideChrome && <header className="rd-header"><div className="rd-identity"><span className="g4-header-face"><img src={leader ? characterArt(leader, 'portrait') : undefined} alt={leader?.name || '先頭武将'} />{activeTab === 'raid' && <b className="rd-raid-header-level">Lv.{state.playerProgress?.level ?? game.userLevel ?? 1}</b>}</span><div>{activeTab !== 'raid' && <small>Lv.{state.playerProgress?.level ?? game.userLevel ?? 1}</small>}<strong>{game.username || 'プレイヤー'} {authenticated ? <span className="rd-auth-badge" aria-label="認証済み">✓</span> : <button className="rd-auth-link" onClick={() => game.setShowAccountAuthenticationModal(true)}>未認証</button>}</strong><span className="rd-guild-slot" aria-label="将来の同盟表示枠">◇ 同盟 —</span></div></div><button className="rd-menu-button" onClick={() => setMenu(true)} aria-label="メニュー">MENU ☰</button><div className="rd-resources"><span><img src="/ui/sengoku/13-coin.png" alt="銭" /> {state.cash.toLocaleString()}</span><span><img src="/ui/sengoku/16-diamond.png" alt="輝石" /> {state.diamonds.toLocaleString()}</span><span><img src="/ui/sengoku/14-energy.png" alt="行動力" /> {state.energy}/{state.energyMax}</span></div></header>}
