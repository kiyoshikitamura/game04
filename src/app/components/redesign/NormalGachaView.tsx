...
export default function NormalGachaView({ data, onAction }: { data: RedesignResponse; onAction: (name: string, payload?: Record<string, unknown>, requestId?: string) => Promise<RedesignResponse> }) {
  ...
  return <section className="rd-panel"><h2>通常登用</h2><p>姫武将・戦技・武具の混合ガチャ</p><p>所持銭 {data.state.cash.toLocaleString()}</p>
    <div className="g4g-actions">{[{count:10,currency:'FREE',label:'毎日1回 無料10連'},{count:1,currency:'CASH',label:'1回 1,000銭'},{count:10,currency:'CASH',label:'10連 10,000銭'}].map(o => <button className="rd-button rd-primary" key={o.label} disabled={busy || !rates.length || (o.currency==='FREE'?!normalState?.available:data.state.cash<o.count*1000)} onClick={() => {pending.current=null;setSelection({count:o.count,currency:o.currency});}}>{o.label}</button>)}</div>
    ...
    {selection && <Modal title="通常登用の確認" onClose={() => { if(!busy){setSelection(null);pending.current=null;} }}><p>{selection.count}回・消費 {cost.toLocaleString()}銭</p><p>所持銭 {data.state.cash.toLocaleString()} → {(data.state.cash-cost).toLocaleString()}</p><button className="rd-button rd-primary" disabled={busy} onClick={() => void draw()}>{busy?'結果を確認中…':'この内容で登用する'}</button>{error&&<p role="alert">{error}</p>}</Modal>}
