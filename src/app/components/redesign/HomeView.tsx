...
export default function HomeView({ state, onAction, onNavigate, encounterRaid, socialEvents = [], missions = [], previewOnly = false }: { state: RedesignState; onAction: HomeAction; onNavigate: (tab: string) => void; encounterRaid?: HomeEncounter | null; socialEvents?: HomeSocialEvent[]; missions?: MissionProjection[]; previewOnly?: boolean }) {
  ...
  return <>
    <div className={`g4-home ${encounterActive ? 'has-encounter' : ''}`} style={{ backgroundImage: `url(${background.image})` }} aria-busy={!homeImages.ready}>
      <div className="g4-home-stage">
        <section className="g4-home-visual" aria-label="お気に入り武将">
          ...
          {!homeImages.ready && <div className="g4-home-loading" role="status">{homeImages.error ? <><span>画像を読み込めませんでした</span><ActionButton className="rd-button" onClick={homeImages.retry}>再読み込み</ActionButton></> : <span className="g4-home-spinner" aria-label="読み込み中" />}</div>}
        </section>
    ...
    {profileId && <Modal title="プロフィール" onClose={() => setProfileId('')} footer={profileId !== state.userId ? <ActionButton className="rd-button" onClick={() => { game.setDmRecipientId(profileId); setProfileId(''); setCommunity('dm'); setExpanded(true); }}>DMを送る</ActionButton> : undefined}>
      {profiles[profileId] ? <><h3>{profileName(profileId, 'プレイヤー')}{badges(profileId)}</h3><p style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{profileId === state.userId ? game.bio || '自己紹介は未設定です。' : profiles[profileId].bio || '自己紹介は未設定です。'}</p></> : <><p>プロフィールを取得できない場合は再読み込みしてください。</p><ActionButton className="rd-button" onClick={() => setActivityAttempt(n => n + 1)}>再読み込み</ActionButton></>}
    </Modal>}
    {expanded && !profileId && <Modal title="コミュニティ" onClose={() => setExpanded(false)} footer={community !== 'activity' && (community !== 'dm' || game.dmRecipientId) ? <form className="rd-row" onSubmit={async e => { e.preventDefault(); setCommunityError(''); try { if (community === 'dm') { if (await game.handleSendDirectMessage(game.dmRecipientId, dmText)) setDmText(''); } else await game.handleSendChat(); } catch { setCommunityError('送信できませんでした。もう一度お試しください。'); } }}><input aria-label="メッセージ" maxLength={COMMUNITY_MESSAGE_MAX_LENGTH} value={community === 'dm' ? dmText : game.chatInput} onChange={e => community === 'dm' ? setDmText(e.target.value) : game.setChatInput(e.target.value)} /><ActionButton type="submit" variant="primary" className="rd-button" disabled={game.chatSending || (community === 'global' && game.chatCooldown > 0) || !(community === 'dm' ? dmText : game.chatInput)?.trim()}>送信</ActionButton></form> : undefined}>{tabs}{activityStatus}{messages(true)}{communityError && <p role="status">{communityError}</p>}</Modal>}
  </>;
}
