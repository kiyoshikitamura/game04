'use client';
import { beginPresentation, usePresentationBusy } from '../ui/presentationTasks';
...
import { raidTimeRemaining } from '@/domain/redesign/raidPresentation';
...
export default function HomeView({ state, onAction, onNavigate, encounterRaid, socialEvents = [], previewOnly = false }: { state: RedesignState; onAction: HomeAction; onNavigate: (tab: string) => void; encounterRaid?: HomeEncounter | null; socialEvents?: HomeSocialEvent[]; previewOnly?: boolean }) {
  ...
  const themedBackground = backgrounds.find(b => b.characterId === favorite.id);
  const selectedBackground = HOME_BACKGROUNDS.find(b => b.id === state.homeBackgroundId) ?? HOME_BACKGROUNDS[0];
  const background = selectedBackground.id === 'castle-approach' && themedBackground ? themedBackground : selectedBackground;
  ...
  useEffect(() => {
    ...
    let cancelled = false; const loading=beginPresentation();
    void supabase.rpc('get_recent_social_activity_feed', { p_limit: 20 }).then(({ data, error }) => { if (!cancelled) { setActivities((data ?? []).filter((item: Activity) => !/PVP|GVG|GUILD/.test(item.activity_type ?? ''))); if (error) setCommunityError('通信状態を確認し、活動を再読み込みしてください。'); } }).then(loading.end,loading.end);
  ...
  async function saveHome(payload: Record<string, unknown>) {
    ...
    try { await onAction('set_home', payload); setSelector(null); } catch (error) { setSaveError(game04UiError(error)); } finally { homeLock.current = false; setSaving(false); }
  }
  ...
  function messages(full: boolean) {
    ...
    if (full && game.dmRecipientId) return <><button className="rd-button" onClick={() => game.setDmRecipientId(null)}>会話一覧へ</button><h3>{activeDm?.userName || 'プレイヤー'}</h3>{direct.map((m: {id:string;sender_name?:string;recipient_id?:string;is_read?:boolean;message?:string;content?:string}) => <ViewedItem className="rd-message" key={m.id} onViewed={()=>{if(!previewOnly&&m.recipient_id===state.userId&&!m.is_read)void game.markDirectMessagesRead?.([m.id]);}}><b>{m.sender_name || 'プレイヤー'}</b><span>{m.message || m.content}</span></ViewedItem>)}</>;
