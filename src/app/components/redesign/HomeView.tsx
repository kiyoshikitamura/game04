...
export default function HomeView({ state, onAction, onNavigate, encounterRaid, socialEvents = [], previewOnly = false }: { state: RedesignState; onAction: HomeAction; onNavigate: (tab: string) => void; encounterRaid?: HomeEncounter | null; socialEvents?: HomeSocialEvent[]; previewOnly?: boolean }) {
  ...
  function messages(full: boolean) {
    if (community !== 'dm') {
      ...
      const merged = [...legacy.map(m => ({ ...m, roomId: '' })), ...socialEvents.map(e => ({ id: e.id, createdAt: e.created_at, author: '援軍要請', body: e.body, roomId: e.room_id, userId: e.author_id }))].sort((a,b) => Date.parse(b.createdAt) - Date.parse(a.createdAt));
      return merged.length ? merged.slice(0, full ? merged.length : 3).map(m => <div className="rd-message" key={m.id}><button className="rd-text-button" onClick={() => { if (m.userId && m.userId !== state.userId) { game.setDmRecipientId(m.userId); setCommunity('dm'); setExpanded(true); } }}>{m.author}</button><div><span>{m.body}</span>{m.roomId && <button className="rd-button rd-rescue-link" onClick={() => { setExpanded(false); onNavigate('raid'); }}>レイドを確認 ›</button>}</div></div>) : <p className="rd-muted">{community === 'activity' ? communityError || '新しい活動はまだありません' : '全体にひとこと送ってみましょう'}</p>;
    }
  ...
  return <>
    ...
    <div className="rd-home-bottom"><button className="rd-quest-resume" onClick={() => onNavigate('quest:resume')}><small>クエスト · {area.name}</small><strong>{area.index}-{stage.index} {stage.name}</strong><span>続きから ›</span></button><section className="rd-community-preview">{tabs}<div className="rd-community-lines">{messages(false)}</div><button className="rd-text-button" onClick={() => setExpanded(true)}>コミュニティを開く ›</button></section><button className="rd-rotation-banner" onClick={() => onNavigate(banner ? 'shop' : 'gacha')}><span>{banner ? '旅の支度を整える' : '新たな力と出会う'}</span><strong>{banner ? '商店' : '登用'} ›</strong></button></div>
    {missionsOpen && <Modal title="攻略の記録" onClose={() => setMissionsOpen(false)}><p>クリア済み {state.clearedStages.length} / {QUEST_AREAS.reduce((sum, a) => sum + a.stages.length, 0)} ステージ</p><div className="rd-stack">{QUEST_AREAS.map(a => { const cleared = a.stages.filter(stage => state.clearedStages.includes(stage.id)).length; return <div className="rd-panel rd-row" key={a.id}><strong>{a.name}</strong><span>{cleared}/{a.stages.length}{cleared === a.stages.length ? ' 達成' : ''}</span></div>; })}</div><p className="rd-muted">達成報酬は準備中です。</p></Modal>}
