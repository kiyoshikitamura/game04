...
export default function CowboyDisplay({characterId,name}:{characterId:string;name:string}) {
  ...
  return <svg className="g4-cowboy" viewBox={`${crop.x} ${crop.y} ${crop.width} ${crop.height}`} preserveAspectRatio="xMidYMin meet" role="img" aria-label={name} data-character={characterId}>
    <image href={source} width={crop.naturalWidth} height={crop.naturalHeight}/>
  </svg>;
}
