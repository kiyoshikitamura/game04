...
const statusNames: Record<string, string> = { atk_up: '攻↑', def_up: '防↑', atk_down: '攻↓', def_down: '防↓', poison: '毒', stun: '行動不能' };
...
const eventText = (text: string) => text.replace(/\b(atk_up|def_up|atk_down|def_down|stun)\b/g, key => statusNames[key]);
...
export function BattleView({ result, vipActive, onComplete, title = '合戦', raidHp, initialFrame = 0, initialPaused = false }: Props) {
  ...
  const unitCard = (state: BattleUnitState, enemy: boolean, order: number) => {
    ...
    return <div className={`${enemy ? styles.enemy : styles.member} ${state.hp <= 0 ? styles.dead : ''} ${frame.actorId === state.id ? styles.active : ''}`} key={state.id}>
      <button className={styles.unitButton} onClick={() => setDetail({ unit, state })} aria-label={`${unit.name}の戦闘詳細`}>
        ...
        <span className={styles.status}>{state.statuses.slice(0, 3).map((s, i) => <span key={i}>{statusNames[s.type] ?? s.type}</span>)}{state.stunImmune && <span title="実際に1回行動するまで行動不能の再付与を防ぎます">耐性</span>}</span>
      </button>
      {!enemy && <span className={styles.skills}>{(state.skills ?? unit.skills).slice(0, 3).map((skill, slot) => {
        ...
        const status = active ? '発動中' : recorded ? readinessNames[recorded.status] ?? recorded.status : '記録なし';
        return <button key={skill.id} className={`${styles.skill} ${active ? styles.skillActive : recorded?.status === 'condition_unmet' ? styles.condition : recorded?.status === 'insufficient_sp' ? styles.shortSp : ''}`} title={`優先${slot + 1} ${skill.name}：${status}`} aria-label={`優先${slot + 1} ${skill.name}：${status}`} onClick={() => setDetail({ skill, readiness: status, cost: recorded?.cost, reason: recorded?.reason })}>
          <img src={skill.image} alt="" /><span>{active ? '発動' : recorded?.status === 'condition_unmet' ? '条件' : recorded?.status === 'insufficient_sp' ? 'SP' : recorded?.status === 'ready' ? '可' : '—'}</span>
        </button>;
      })}</span>}
    </div>;
  };
  return <section className={styles.battle} aria-label={title}>
    ...
    {(detail || showLog) && <div className={styles.backdrop} onClick={close}><section className={styles.modal} role="dialog" aria-modal="true" aria-label={showLog ? '戦闘ログ' : '戦闘詳細'} onClick={e => e.stopPropagation()}><button className={styles.close} onClick={close} autoFocus>閉じる</button>{showLog ? <>
      <h2>戦闘ログ</h2>{result.frames.map(f => <article className={styles.logEntry} key={f.index}><strong>#{f.index + 1} W{f.wave} {f.actorId ? lookup(f.actorId, f.wave)?.name ?? f.actorId : ''}</strong><p>{eventText(f.text)}</p>{f.targetIds?.length ? <p>対象：{f.targetIds.map(id => lookup(id, f.wave)?.name ?? id).join('、')}</p> : null}<p>SP {f.partySp}/{f.maxSp}{f.spDelta !== undefined ? ` (${signed(f.spDelta)})` : ''}{f.burstGauge !== undefined ? ` · ゲージ ${f.burstGauge}/${f.maxBurstGauge ?? 200}` : ''}{f.gaugeDelta !== undefined ? ` (${signed(f.gaugeDelta)})` : ''}{f.remainingActions !== undefined ? ` · 残り${f.remainingActions}回` : ''}</p>{f.hits && f.hits.length > 1 && <p>ヒット表示：{f.hits.join(' + ')}</p>}{f.reason && <p>{reasonText(f.reason)}</p>}</article>)}
    </> : detail?.skill ? <><h2>{detail.skill.name}</h2><p>{detail.readiness}{detail.cost !== undefined ? ` · 今回の消費SP ${detail.cost}` : ''}</p>{detail.reason && <p>{detail.reason}</p>}<p>基本消費SP {detail.skill.spCost} · {elements[detail.skill.element]}</p><p>{detail.skill.description}</p><p>条件：{conditionText(detail.skill)}</p></> : detail?.unit && <><h2>{detail.unit.name}</h2><p>HP {detail.state?.hp} / {detail.state?.maxHp}</p>{detail.state && result.waves[frame.wave - 1]?.some(u => u.id === detail.unit?.id) && <p>SP {detail.state.sp} / {detail.state.maxSp} · あと {detail.state.count} 行動で割り込み</p>}<h3>パッシブ</h3>{detail.unit.passives.map((p, i) => <p key={`${p.id}-${i}`}>{p.name} · {p.stat.toUpperCase()} +{p.percent}%</p>)}<h3>スキル発動優先順</h3>{(detail.state?.skills ?? detail.unit.skills).map((s, slot) => <p key={s.id}><strong>優先{slot + 1}：{s.name}</strong>（SP {s.spCost}）<br />{conditionText(s)}<br />{s.description}</p>)}<h3>状態</h3>{detail.state?.statuses.length ? detail.state.statuses.map((s, i) => <p key={i}>{statusNames[s.type] ?? s.type} {s.type === 'stun' ? '' : `${s.power}%`} · 残り{s.remaining}行動</p>) : <p>なし</p>}{detail.state?.stunImmune && <p>行動不能の再付与耐性：次の実行行動完了まで</p>}</>}</section></div>}
  </section>;
}
