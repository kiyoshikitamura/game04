  ...
  return <div className="redesign-quest-dialog">
    <CanonicalDialog title="出撃準備" onClose={busy ? undefined : onBack} actions={[
      ...
      <div className="rq-party">{party.map((unit, index) => { const master = CHARACTER_MASTERS.find(entry => entry.id === unit.id); const subject = master ? { id: master.id, name: unit.name, rarity: master.rarity, element: unit.element as 'fire'|'water'|'earth'|'wind'|'light'|'dark' } : null; return <button type="button" key={unit.id} className="rq-party-card" onClick={() => setDetail(unit)} aria-label={`${index + 1}番 ${unit.name}のスキル・パッシブ`}>
        <span className="rq-order">{index + 1}</span>{subject ? <CharacterCard subject={subject} compact className="rq-party-visual-card" /> : <div className="rq-card-visual"><img className="rq-card-person" src={unit.image} alt={unit.name} /></div>}<div className="rq-party-meta"><strong>{unit.name}</strong><span className="rq-rarity-line"><b>{master?.rarity ?? 'N'}</b> Lv.{unit.level}</span><small>HP {unit.stats.hp.toLocaleString()}</small></div>
      </button>; })}</div>
    ...
    {detail && <CanonicalDialog title={`${detail.name}の詳細`} onClose={() => setDetail(null)} actions={[{ label: '閉じる', onClick: () => setDetail(null) }]}>
      <div className="rq-detail-profile"><CharacterCard subject={{id: detail.id, name: detail.name, rarity: CHARACTER_MASTERS.find(entry => entry.id === detail.id)?.rarity ?? 'N', element: detail.element as 'fire'|'water'|'earth'|'wind'|'light'|'dark'}} compact className="rq-detail-card" /><div><h3>{detail.name}</h3><p className="rq-rarity-line"><b>{CHARACTER_MASTERS.find(entry => entry.id === detail.id)?.rarity ?? 'N'}</b> <ElementBadge element={detail.element} /> · Lv.{detail.level}</p><p>HP {detail.stats.hp.toLocaleString()}</p></div></div>
