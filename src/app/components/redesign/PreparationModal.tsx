  ...
  return <div className="redesign-quest-dialog">
    <CanonicalDialog title="出撃準備" onClose={busy ? undefined : onBack} actions={[
      ...
      <div className="rq-party">{party.map((unit, index) => { const master = CHARACTER_MASTERS.find(entry => entry.id === unit.id); const subject = master ? { id: master.id, name: unit.name, rarity: master.rarity, element: unit.element as 'fire'|'water'|'earth'|'wind'|'light'|'dark' } : null; return <button type="button" key={unit.id} className="rq-party-card" onClick={() => setDetail(unit)} aria-label={`${index + 1}番 ${unit.name}のスキル・パッシブ`}>
        <span className="rq-order">{index + 1}</span><div className="rq-party-visual-wrap">{subject ? <CharacterCard subject={subject} compact hideMarks className="rq-party-visual-card" /> : <div className="rq-card-visual"><img className="rq-card-person" src={unit.image} alt={unit.name} /></div>}<div className="rq-party-badges"><RarityBadge rarity={master?.rarity ?? 'N'} /><ElementBadge element={unit.element} /></div></div><div className="rq-party-meta"><strong>{unit.name}</strong><span className="rq-rarity-line">Lv.{unit.level}</span><small>HP {Math.floor(unit.stats.hp).toLocaleString()}</small></div>
      </button>; })}</div>
    ...
    {detail && <CanonicalDialog title={`${detail.name}の詳細`} onClose={() => setDetail(null)} actions={[{ label: '閉じる', onClick: () => setDetail(null) }]}>
      ...
      <dl className="rq-detail-stats">{Object.entries(detail.stats).map(([key, value]) => <div key={key}><dt>{key.toUpperCase()}</dt><dd>{(['hp', 'atk', 'def', 'luk'].includes(key) ? Math.floor(value) : value).toLocaleString()}</dd></div>)}</dl>
