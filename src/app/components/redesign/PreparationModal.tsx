...
import { characterArt, characterBackground, characterFrame } from '@/theme/creativeAssets';
  ...
  return <div className="redesign-quest-dialog">
    <CanonicalDialog title="出撃準備" onClose={busy ? undefined : onBack} actions={[
      ...
      <div className="rq-party">{party.map((unit, index) => { const master = CHARACTER_MASTERS.find(entry => entry.id === unit.id); return <button type="button" key={unit.id} className="rq-party-card" onClick={() => setDetail(unit)} aria-label={`${index + 1}番 ${unit.name}のスキル・パッシブ`}>
        <span className="rq-order">{index + 1}</span><div className="rq-card-visual"><img className="rq-card-background" src={characterBackground(unit)} alt="" aria-hidden="true" /><img className="rq-card-person" src={characterArt(unit, 'card') ?? unit.image} alt="" /><img className="rq-card-frame" src={characterFrame(master?.rarity ?? 'N')} alt="" /><ElementBadge element={unit.element} className="rq-card-element" /></div><div className="rq-party-meta"><strong>{unit.name}</strong><span className="rq-rarity-line"><b>{master?.rarity ?? 'N'}</b> Lv.{unit.level}</span><small>HP {unit.stats.hp.toLocaleString()}</small></div>
      </button>; })}</div>
