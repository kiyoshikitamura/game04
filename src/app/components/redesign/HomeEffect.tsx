...
export default function HomeEffect({ backgroundImage, effectId: explicitEffectId }: { backgroundImage?: string; effectId?: string }) {
  ...
  const effectId = explicitEffectId ?? (legacySakura ? 'char_kaede_01' : (SSR_HOME_BACKGROUNDS.find(entry=>entry.image===backgroundImage) ?? backgrounds.find(entry=>entry.image===backgroundImage))?.characterId);
  ...
  return enabled && effectId ? <iframe key={effectId} className="g4-home-effect" style={{ colorScheme: 'normal', background: 'transparent' }} src={`/creative/effects/${effectId}.html`} title="背景演出" tabIndex={-1} aria-hidden="true" sandbox="allow-scripts" /> : null;
}
