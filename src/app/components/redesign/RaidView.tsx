...
export default function RaidView({state,rooms,party,onAction,onOpenDeck,initialRoomId}:RaidViewProps){
 ...
 function visual(value:RaidRoom){
  ...
  return {bossName:m.name,characterUrl:characterArt(e,'full'),backgroundUrl:m.backgroundUrl,ownerName:owner?.name??'開催者情報を取得できません',ownerImageUrl:owner?.portraitUrl,
   attributeLabel:raidElementLabels[e.element],attributeIconUrl:`/creative/ui/element-${e.element}.png`,levelLabel:`Lv.${value.level}`,hpPercent:percent,
 ...
 <div className="rd-raid-detail-stage" style={{paddingTop:compact?Math.max(0,summaryHeight-160):0}}>
 ...
 challenge={available?<RaidApprovedChallenge><button disabled={busy||(!me&&room.participants.filter(p=>!p.leftAt).length>=master.maxParticipants)} onClick={()=>me?setModal('prepare'):void run('raid_join')}><RaidApprovedIcon name="swords"/><strong>{me?'挑む':'参加する'}</strong>{me&&<span><img src="/ui/sengoku/14-energy.png" alt=""/>消費行動力 {master.energyCost}</span>}</button></RaidApprovedChallenge>:<button className="rd-raid-history" disabled={busy} onClick={()=>setModal('rewards')}>報酬を確認する</button>}/>
 </div>
