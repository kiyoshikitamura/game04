...
export default function RaidView({state,rooms,party,onAction,onOpenDeck,initialRoomId}:RaidViewProps){
 ...
 {!room||!master||!enemy?<><div className="rd-raid-heading"><h1>レイド</h1><button disabled={busy} onClick={()=>void run('raid_refresh')}>更新</button></div><nav className="rd-raid-tabs" aria-label="レイド種別">{[['all','すべて'],['encounter','エンカウント'],['unlock','領土侵攻']].map(([id,label])=><button key={id} aria-pressed={filter===id} disabled={busy} onClick={()=>setFilter(id)}>{label}</button>)}</nav><div className="rd-raid-cards">{live.length?live.map(renderCard):<p className="rd-panel">開催中のレイドはありません。</p>}</div><button className="rd-raid-history" disabled={busy} onClick={()=>setModal('history')}><RaidApprovedIcon name="chest"/>終了したレイド・未受取報酬<span>›</span></button></>:<>
 <button className="rd-raid-back" disabled={busy} onClick={()=>setSelected(null)}>‹ レイド一覧</button>
 ...
 {modal==='level'&&room&&master&&<CanonicalDialog title="挑戦する段階" onClose={()=>setModal(null)} actions={[{label:'出撃準備へ',semantic:'primary',onClick:()=>setModal('prepare')}]}><label>領土侵攻 Lv.<select aria-label="挑戦するレイド段階" value={battleLevel??room.level} onChange={event=>setBattleLevel(Number(event.target.value))}>{Array.from({length:Math.max(1,room.level-(me?.joinedLevel??1)+1)},(_,index)=>index+(me?.joinedLevel??1)).reverse().map(level=><option key={level} value={level}>{level}{level===room.level?'（現在）':'（再挑戦）'}</option>)}</select></label></CanonicalDialog>}
 {modal==='prepare'&&room&&master&&<PreparationModal title={`${raidEnemy(master,battleLevel??room.level).name} Lv.${battleLevel??room.level} · ${raidElementLabels[raidEnemy(master,battleLevel??room.level).element]}属性`} party={room.territorySnapshot ? buildBattleParty(state, room.territorySnapshot.battleRules) : party} commonSpMax={room.territorySnapshot && room.territorySnapshot.battleRules.version !== 'common-v2-20260920' ? null : 400} energy={state.energy} energyCost={master.energyCost} busy={busy} error={error} onBack={()=>setModal(null)} onOpenDeck={()=>{setModal(null);onOpenDeck();}} onConfirm={()=>void run('raid_battle',{level:battleLevel??room.level})}/>}
 </section>;
}
