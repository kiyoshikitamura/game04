...
export default function RaidView({state,rooms,party,onAction,onOpenDeck,initialRoomId,initialPreparationLevel}:RaidViewProps){
 ...
 function visual(value:RaidRoom){
  ...
  return {bossName:raidDisplayTitle(m),subtitle:raidDisplaySubtitle(m,value.level),characterUrl:characterArt(e,'full'),backgroundUrl:raidBackground(m,value.level)??'/bg/raid/raid-castle-moonlight-v1.webp',ownerName:profiles[value.ownerId]?.username??owner?.name??'開催者情報を取得できません',ownerBadges:<CommunityBadges authenticated={profiles[value.ownerId]?.authenticated} vipExpiresAt={profiles[value.ownerId]?.vip_expires_at} now={now}/>,ownerImageUrl:owner?.portraitUrl,
 ...
 {!room||!master||!enemy?<><div className="rd-raid-heading"><PageTitleBanner page="raid"/><ActionButton disabled={busy} onClick={()=>void run('raid_refresh')}>更新</ActionButton></div><nav className="rd-raid-tabs" aria-label="共闘種別">{[['all','すべて'],['encounter','エンカウント'],['unlock','領土侵攻']].map(([id,label])=><button key={id} aria-pressed={filter===id} disabled={busy} onClick={()=>setFilter(id)}>{label}</button>)}</nav><div className="rd-raid-cards">{live.length?live.map(renderCard):<p className="rd-panel">開催中の共闘はありません。</p>}</div><button className="rd-raid-history" disabled={busy} onClick={()=>setModal('history')}><RaidApprovedIcon name="chest"/>終了した共闘・未受取報酬<span>›</span></button></>:<>
 <ActionButton className="rd-raid-back" disabled={busy} onClick={()=>setSelected(null)}>‹ 共闘一覧</ActionButton>
 ...
 challenge={available?<RaidApprovedChallenge><button disabled={busy||(!me&&room.participants.filter(p=>!p.leftAt).length>=master.maxParticipants)} onClick={()=>{if(!me){void run('raid_join');return;}setBattleLevel(room.level);setModal(master.type==='unlock'&&room.level>(me.joinedLevel??1)?'level':'prepare');}}><RaidApprovedIcon name="swords"/><strong>{me?'挑む':'参加する'}</strong>{me&&<span><img src="/ui/sengoku/14-energy.png" alt=""/>消費行動力 {master.energyCost}</span>}</button></RaidApprovedChallenge>:<ActionButton className="rd-raid-history" disabled={busy} onClick={()=>setModal('rewards')}>報酬を確認する</ActionButton>}/>
 </div>
 {me&&!me.leftAt&&room.ownerId!==state.userId&&active(room)&&<ActionButton className="rd-button" disabled={busy} onClick={()=>setModal('leave')}>この共闘から退出</ActionButton>}
 </>}
 {modal==='history'&&<CanonicalDialog title="終了した共闘・未受取報酬" onClose={busy?undefined:()=>setModal(null)}>{history.length?history.map(r=><button className="rd-button" key={r.id} onClick={()=>select(r)}>{raidDisplayLabel(getRoomRaidMaster(r),r.level)} · {r.status==='defeated'?'討伐成功':'終了'}{r.rewardGrants.some(g=>g.userId===state.userId&&!g.claimed)?' · 未受取あり':''}</button>):<p>終了した共闘はありません。</p>}</CanonicalDialog>}
 {room&&master&&enemy&&modal==='enemy'&&<CanonicalDialog title="敵情報" onClose={busy?undefined:()=>setModal(null)}>{raidEnemies(master,room.level).map(unit=><section key={unit.id}><h3>{enemyRoleLabel(unit)} {unit.name} Lv.{unit.level} · {raidElementLabels[unit.element]}属性</h3><p>HP {unit.stats.hp.toLocaleString()} / ATK {unit.stats.atk.toLocaleString()} / DEF {unit.stats.def.toLocaleString()}</p><p>開始SP {unit.initialSp??unit.stats.sp} / 上限 {unit.stats.sp} · 行動カウント {unit.actionCount}</p>{unit.skills.length>0&&<><h4>所持スキル</h4>{unit.skills.map(skill=><p key={skill.id}>{skill.name} · 消費SP {skill.spCost}<br/>{displaySkillDescription(skill.description)}</p>)}</>}{!!unit.phases?.length&&<><h4>フェーズチェンジ</h4>{unit.phases.map(phase=><p key={phase.name}>HP {Math.round(phase.hpBelow*100)}%以下：{phase.name} · 行動カウント {phase.actionCount??unit.actionCount}</p>)}</>}</section>)}<p>勝利時、共通HPへの与ダメージは{master.victoryMultiplier}倍になります。</p></CanonicalDialog>}
 {room&&modal==='participants'&&<CanonicalDialog title="参加者" onClose={busy?undefined:()=>setModal(null)}><ul className="rd-raid-participants">{room.participants.map(p=><li key={p.userId}><strong>{profiles[p.userId]?.username??p.name}<CommunityBadges authenticated={profiles[p.userId]?.authenticated} vipExpiresAt={profiles[p.userId]?.vip_expires_at} now={now}/>{p.userId===state.userId?'（自分）':''}</strong><span>{p.wins}勝 / {p.totalDamage.toLocaleString()}ダメージ{p.leftAt?' · 退出済み':''}</span></li>)}</ul></CanonicalDialog>}
 {room&&master&&modal==='rewards'&&<CanonicalDialog title="報酬" onClose={busy?undefined:()=>setModal(null)} actions={[{label:'まとめて受け取る',semantic:'primary',disabled:busy||pendingRewards.length===0,onClick:()=>run('raid_claim')}]}>
  {error&&<p role="alert" className="rd-raid-error">{error}</p>}
  <section aria-label="受取可能な報酬"><h3>受取可能な報酬（{pendingRewards.length}件）</h3>{pendingRewards.length?pendingRewards.map(grant=><section key={grant.id}><h4>{grant.id.startsWith('participation:')?'参加報酬':master.type==='unlock'?`Lv.${grant.level} 討伐報酬`:'討伐報酬'}</h4><Rewards rewards={grant.rewards}/></section>):<p>受取可能な報酬はありません。</p>}</section>
  <details><summary>{master.type==='unlock'?`現在のLv.${room.level}の報酬・獲得条件`:'報酬の内容・獲得条件'}</summary>
   {master.participationRewards.length>0&&<><h3>参加報酬</h3><Rewards rewards={master.participationRewards}/><p>初回戦闘後に受け取れます。</p></>}
   {!!master.victoryRewards?.length&&<><h3>個別バトル勝利報酬</h3><Rewards rewards={master.victoryRewards}/>{master.victoryRewards.some(reward=>reward.chance!==undefined)&&<p>確率付きの報酬は抽選です。LUKにより獲得率が上がります。</p>}</>}
   <h3>{master.type==='unlock'?`Lv.${room.level} 討伐時の報酬`:'討伐時の報酬'}</h3><Rewards rewards={master.defeatRewards}/>
   <p>{(me?.wins??0)>=3?'討伐報酬資格 取得済み':`あと${3-(me?.wins??0)}勝で討伐報酬資格`}</p><p>討伐報酬は資格取得後に討伐された敵が対象です。</p>{master.type==='unlock'&&<p>参加前・資格取得前に突破済みのLv報酬は配布されません。</p>}
  </details>
 </CanonicalDialog>}
 {room&&master&&window&&modal==='rescue'&&<CanonicalDialog title="救援依頼" onClose={busy?undefined:()=>setModal(null)} actions={[{label:'救援を依頼',semantic:'primary',disabled:busy||!available||!me||window.remaining===0,onClick:()=>run('raid_rescue')}]}>{error&&<p role="alert" className="rd-raid-error">{error}</p>}<p>{raidDisplayLabel(master,room.level)}</p><p>残り {window.remaining} / 3回</p><p>公開先：全体チャット・アクティビティ</p>{window.resetsAt?<p>次回回復：{raidTimeRemaining(window.resetsAt,now)}後（6時間ごと）</p>:<p>この共闘で合計3回まで依頼できます。</p>}</CanonicalDialog>}
