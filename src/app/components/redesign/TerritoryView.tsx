...
export default function TerritoryView({ territory, rooms, userId, onHost, onOpenRoom }: {
  ...
  return <section className="rd-stack rd-territory">
    <h1>領土侵攻</h1><p>他領地の城に挑み、仲間とともに制圧を目指しましょう。</p>
    {territory ? <section className="rd-panel"><h2>領土侵攻の主催者Lv. {territory.level}</h2><p>主催者EXP {territory.experience.toLocaleString()}{territory.nextLevelExp != null ? ` ／ 次の主催者Lvまで ${Math.max(0, territory.nextLevelExp - territory.experience).toLocaleString()}` : ' ／ 上限到達'}</p><p>同時開催枠 {territory.activeHostingCount} / {territory.hostingSlots}</p><p className="rd-muted">主催した侵攻を最終ボスまでクリアし、その時点で個別バトルに3勝していると主催者EXPを獲得します。</p></section> : <p className="rd-panel" role="status">開催条件を読み込めませんでした。画面を開き直してください。</p>}
    ...
    {territory?.destinations.map(entry => <section key={entry.id} className="rd-panel rd-territory-destination">
      ...
      <div className="rd-territory-conditions"><p>開催期間 {duration(entry.durationMinutes)} · 最終ボスLv.{entry.raidMaster.maxLevel}</p><p>必要主催者Lv. {entry.requiredLevel}</p><p>開催アイテム {entry.itemName} ×{entry.itemCount}（所持 {entry.ownedItemCount}）</p><p>入手方法：{entry.itemSource}</p><h4>参加報酬</h4><ul>{entry.raidMaster.participationRewards.map((reward, index) => <li key={index}>{raidRewardLabel(reward)}</li>)}</ul><h4>各ボスLvの討伐報酬</h4><ul>{entry.raidMaster.defeatRewards.map((reward, index) => <li key={index}>{raidRewardLabel(reward)}</li>)}</ul><p>侵攻クリア時の主催者EXP {entry.clearExp.toLocaleString()}（クリア時3勝が必要）</p>
