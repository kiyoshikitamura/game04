...
export function LevelGrowthControls({ state, kind, id, run, busy, portrait, onCancel }: { state: RedesignState; kind: GrowthKind; id: string; run: Run; busy: boolean } & Presentation) {
  ...
  return <fieldset className="g4g-growth-controls" disabled={busy} aria-label="Lv育成の内容">
    <div className="g4g-growth-hero"><div className="g4g-growth-portrait">{portrait ?? (master && 'image' in master ? <img src={master.image} alt={master.name}/> : null)}</div><div className="g4g-growth-summary"><h3>{master?.name}</h3><p><RarityBadge rarity={master?.rarity??''}/>{master && 'element' in master ? <ElementBadge element={master.element}/> : null}</p><div className="g4g-growth-transition">Lv.{owned?.level} <span>→</span> <strong>Lv.{quote?.levelAfter ?? owned?.level}</strong></div><p>上限 {cap} / 最大 100</p></div></div>
    <div className="g4g-growth-section-head"><strong>経験値素材を選択</strong><ActionButton size="compact" type="button" onClick={() => setItems({ ...inventory.expItems[kind] })}>一括選択</ActionButton><ActionButton size="compact" type="button" onClick={() => setItems({})}>解除</ActionButton></div>
    ...
    <div className="g4g-growth-footer">{onCancel && <ActionButton type="button" onClick={onCancel}>キャンセル</ActionButton>}<ActionButton variant="primary" type="button" className="g4g-primary" disabled={!quote || state.cash < cash} onClick={() => void run(`${kind}_level`, { [kind === 'character' ? 'characterId' : 'instanceId']: id, items })}>育成する</ActionButton></div>
  </fieldset>;
}
...
export function SoulControls({ state, id, run, busy, mode = 'awaken', portrait, onCancel }: { state: RedesignState; id: string; run: Run; busy: boolean; mode?: 'awaken' | 'exchange' } & Presentation) {
  ...
  return <fieldset className="g4g-growth-controls" disabled={busy} aria-label="魂・覚醒">
    <div className="g4g-growth-hero"><div className="g4g-growth-portrait">{portrait}</div><div className="g4g-growth-summary"><h3>{master.name}</h3><p className="g4-identity-badges"><RarityBadge rarity={master.rarity}/><ElementBadge element={master.element}/></p>{mode === 'awaken' && owned && cost > 0 && <><div className="g4g-growth-transition">覚醒 <strong>+{owned.awakening} → +{owned.awakening + 1}</strong></div><table className="g4g-growth-comparison"><tbody><tr><th>Lv上限</th><td>{getCharacterLevelCap(owned.awakening)}</td><td>→</td><td>{getCharacterLevelCap(owned.awakening + 1)}</td></tr><tr><th>スキル枠</th><td>{getSkillSlots(owned.awakening)}</td><td>→</td><td>{getSkillSlots(owned.awakening + 1)}</td></tr>{master.rarity !== 'N' && <tr><th>パッシブLv</th><td>{owned.awakening * 2}</td><td>→</td><td>{(owned.awakening + 1) * 2}</td></tr>}</tbody></table></>}</div></div>
    {mode === 'awaken' && (!owned ? <><dl className="g4g-growth-ledger"><div><dt>固有魂（所持 / 必要）</dt><dd><strong>{souls} / {unlockCost}</strong></dd></div></dl><p className="g4g-growth-note">解放時 Lv.1・覚醒 +0・銭不要<br/>固有魂のみ使用できます。</p>{souls < unlockCost && <p role="status">固有魂があと{unlockCost - souls}個必要です。</p>}<div className="g4g-growth-footer">{onCancel && <ActionButton type="button" onClick={onCancel}>キャンセル</ActionButton>}<ActionButton variant="primary" type="button" className="g4g-primary" disabled={souls < unlockCost} onClick={() => void run('character_unlock', { characterId: id })}>解放する</ActionButton></div></> : cost > 0 ? <><div className="g4g-growth-section-head"><strong>必要な魂</strong><span>必要数 {specific + common} / {cost}</span></div><div className="g4g-soul-grid"><div className="g4g-soul-choice"><strong>固有魂</strong><small>所持 {souls}</small><Stepper label="覚醒で使う固有魂" value={specific} max={Math.min(souls, cost)} onChange={setSpecific}/></div><div className="g4g-soul-choice"><img src={growthSoulImage('generic_soul', master.rarity)} alt="" width={40} height={40} style={{objectFit:'contain'}}/><strong>{master.rarity}汎用魂</strong><small>所持 {generic}</small><Stepper label="覚醒で使う汎用魂" value={common} max={Math.min(generic, cost)} onChange={n => setSpecific(cost - n)}/></div></div><dl className="g4g-growth-ledger"><div><dt>消費銭</dt><dd>{(cost * 2000).toLocaleString()}</dd></div><div><dt>所持銭</dt><dd>{state.cash.toLocaleString()} → {(state.cash - cost * 2000).toLocaleString()}</dd></div></dl><small className="g4g-growth-note">現在Lv.{owned.level}・EXPは変わりません</small>{common > generic && <p role="status">汎用魂が不足しています。</p>}{specific > souls && <p role="status">固有魂が不足しています。</p>}{state.cash < cost * 2000 && <p role="status">銭が不足しています。</p>}<div className="g4g-growth-footer">{onCancel && <ActionButton type="button" onClick={onCancel}>キャンセル</ActionButton>}<ActionButton variant="primary" type="button" className="g4g-primary" disabled={awakeningDisabled} onClick={async () => { if (await run('character_awaken', { characterId: id, specificSouls: specific, genericSouls: common })) setSpecific(null); }}>覚醒する</ActionButton></div></> : <p>最大覚醒です。</p>)}
    {mode === 'exchange' && <><h4>固有魂を汎用魂へ交換</h4><label>固有魂の交換数（10個以上・2個単位）<input aria-label="固有魂の交換数" type="number" min={10} max={souls - souls % 2} step={2} value={amount} onChange={e => { setAmount(Number(e.target.value)); setConfirm(null); }}/></label><dl className="g4g-growth-ledger"><div><dt>固有魂</dt><dd>{souls} → {Math.max(0, souls - amount)}</dd></div><div><dt><img src={growthSoulImage('generic_soul', master.rarity)} alt="" width={28} height={28} style={{objectFit:'contain',verticalAlign:'middle'}}/>{master.rarity}汎用魂</dt><dd>{generic} → {generic + amount / 2}</dd></div></dl><p>銭不要</p><ActionButton type="button" disabled={!Number.isSafeInteger(amount) || amount < 10 || amount % 2 !== 0 || amount > souls} onClick={() => setConfirm('exchange')}>交換内容を確認</ActionButton>{souls < 10 && <p role="status">交換には固有魂が10個以上必要です。</p>}<h4><img src={growthSoulImage('soul_selector', master.rarity)} alt="" width={32} height={32} style={{objectFit:'contain',verticalAlign:'middle'}}/>{master.rarity}魂選択アイテム：{inventory.soulSelectors[master.rarity]}個</h4><p>1個 → {master.name}の固有魂10個</p><ActionButton type="button" disabled={!owned || owned.awakening >= 5 || inventory.soulSelectors[master.rarity] < 1} onClick={() => setConfirm('select')}>この武将の魂を選ぶ</ActionButton>{onCancel && !confirm && <ActionButton type="button" onClick={onCancel}>キャンセル</ActionButton>}</>}
    {confirm && <div role="group" aria-label="魂操作の確認"><p>{confirm === 'exchange' ? `固有魂${amount}個を汎用魂${amount / 2}個へ交換します。元には戻せません。` : `選択アイテム1個を${master.name}の固有魂10個へ変換します。`}</p><div className="g4g-growth-footer"><ActionButton type="button" onClick={() => setConfirm(null)}>取消</ActionButton><ActionButton variant="primary" type="button" className="g4g-primary" onClick={async () => { if (await run(confirm === 'exchange' ? 'soul_exchange' : 'soul_select', { characterId: id, amount: confirm === 'exchange' ? amount : 1 })) { setConfirm(null); setAmount(10); } }}>確定する</ActionButton></div></div>}
  </fieldset>;
}
...
export function GrowthInventoryView({ state }: { state: RedesignState }) {
  ...
  return <><button type="button" className="g4-inventory-entry" onClick={()=>setOpen(true)}><img src={growthExpImage('character','small')} alt=""/><span>育成アイテム<span>所持品を確認</span></span><b aria-hidden="true">›</b></button>
    {open&&<CanonicalDialog title="育成アイテム" density="compact" actions={[{label:'閉じる',onClick:()=>setOpen(false)}]}>
      ...
      <RewardList items={items}/>
      {(category==='character'||category==='equipment')&&<p>繰越EXP：{inventory.carryExp[category].toLocaleString()}</p>}
    </CanonicalDialog>}
  </>;
}
