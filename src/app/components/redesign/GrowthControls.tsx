...
const materialPaths: Record<string, string> = { character_small: '/items/char_exp_s.png', character_medium: '/items/char_exp_m.png', character_large: '/items/char_exp_l.png', equipment_small: '/items/equip_exp_s.png', equipment_medium: '/items/equip_exp_m.png', equipment_large: '/items/equip_exp_l.png' };
...
export function SoulControls({ state, id, run, busy, mode='awaken' }: { state: RedesignState; id: string; run: Run; busy: boolean; mode?: 'awaken'|'exchange' }) {
  ...
  return <fieldset disabled={busy}><legend>魂・覚醒</legend><p>固有魂 {souls}・{master.rarity}汎用魂 {generic}</p>
    {mode==='awaken'&&(!owned ? <><p>初回解放：固有魂 {SOUL_UNLOCK[master.rarity]}個・銭不要。汎用魂は使えません。</p><button disabled={souls < SOUL_UNLOCK[master.rarity]} onClick={() => void run('character_unlock', { characterId: id })}>固有魂で武将を迎える</button>{souls < SOUL_UNLOCK[master.rarity] && <p role="status">固有魂があと{SOUL_UNLOCK[master.rarity] - souls}個必要です。</p>}</> : cost > 0 ? <><p>覚醒+{owned.awakening} → +{owned.awakening + 1}・必要魂{cost}・必要銭{(cost * 2000).toLocaleString()}</p><label>固有魂を使う数<input aria-label="覚醒で使う固有魂" type="number" min={0} max={Math.min(souls, cost)} step={1} value={specific} onChange={e => setSpecific(Number(e.target.value))}/></label><p>汎用魂 {common}個を併用</p><p>Lv上限 {50 + owned.awakening * 10} → {60 + owned.awakening * 10}・スキル枠 {getSkillSlots(owned.awakening)} → {getSkillSlots(owned.awakening + 1)}{master.rarity !== 'N' && `・パッシブLv ${owned.awakening * 2} → ${(owned.awakening + 1) * 2}`}</p><button disabled={!Number.isSafeInteger(specific) || specific < 0 || specific > souls || common < 0 || common > generic || state.cash < cost * 2000} onClick={async () => { if (await run('character_awaken', { characterId: id, specificSouls: specific, genericSouls: common })) setSpecific(null); }}>この内訳で覚醒する</button>{common > generic && <p role="status">汎用魂が不足しています。</p>}{state.cash < cost * 2000 && <p role="status">銭が不足しています。</p>}</> : <p>最大覚醒です。</p>)}
    {mode==='exchange'&&<><h4>固有魂を汎用魂へ交換</h4><label>固有魂の交換数（10個以上・2個単位）<input aria-label="固有魂の交換数" type="number" min={10} max={souls - souls % 2} step={2} value={amount} onChange={e => { setAmount(Number(e.target.value)); setConfirm(null); }}/></label>
    <p>固有魂 {souls} → {Math.max(0, souls - amount)}・{master.rarity}汎用魂 {generic} → {generic + amount / 2}・銭不要</p><button disabled={!Number.isSafeInteger(amount) || amount < 10 || amount % 2 !== 0 || amount > souls} onClick={() => setConfirm('exchange')}>交換内容を確認</button>
    {souls < 10 && <p role="status">交換には固有魂が10個以上必要です。</p>}<h4>{master.rarity}魂選択アイテム：{inventory.soulSelectors[master.rarity]}個</h4><p>1個 → {master.name}の固有魂10個</p><button disabled={!owned || owned.awakening >= 5 || inventory.soulSelectors[master.rarity] < 1} onClick={() => setConfirm('select')}>この武将の魂を選ぶ</button>
    </>}{confirm && <div role="group" aria-label="魂操作の確認"><p>{confirm === 'exchange' ? `固有魂${amount}個を汎用魂${amount / 2}個へ交換します。元には戻せません。` : `選択アイテム1個を${master.name}の固有魂10個へ変換します。`}</p><button onClick={async () => { if (await run(confirm === 'exchange' ? 'soul_exchange' : 'soul_select', { characterId: id, amount: confirm === 'exchange' ? amount : 1 })) { setConfirm(null); setAmount(10); } }}>確定する</button><button onClick={() => setConfirm(null)}>取消</button></div>}
  </fieldset>;
}
