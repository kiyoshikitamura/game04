...
export default function InventoryView({state,onAction,onNavigate,fixtureItems}:{state:RedesignState;onAction:(name:string,payload:Record<string,unknown>)=>Promise<unknown>;onNavigate:(tab:string)=>void;fixtureItems?:StoredItem[]}) {
  ...
  const visible=rows.filter(r=>(!category||r.category===category)&&r.name.normalize('NFKC').includes(query.normalize('NFKC').trim())).sort((a,b)=>sort==='amount'?b.amount-a.amount:a.name.localeCompare(b.name,'ja'));
  const remote=!category||category==='召喚券'||category==='保管品';
