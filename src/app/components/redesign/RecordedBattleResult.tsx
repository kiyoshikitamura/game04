...
import {StatGrid,InfoRow} from '../ui/Game04DataDisplay';
...
export default function RecordedBattleResult({result,title,backgroundSrc,actions,rewards}:{result:BattleResult;title:string;backgroundSrc:string;actions:ReactNode;rewards?:ReactNode}) {
 ...
 useEffect(()=>{const target=mvp?.score.total??0;if(matchMedia('(prefers-reduced-motion: reduce)').matches){setScore(target);return;}setScore(0);const begin=performance.now();let id=0;const tick=(t:number)=>{const progress=Math.min(1,(t-begin)/UI_MOTION.countMs);setScore(Math.round(target*progress));if(progress<1)id=requestAnimationFrame(tick);};id=requestAnimationFrame(tick);return()=>cancelAnimationFrame(id);},[result,mvp?.score.total]);
 return <section className="recorded-result" style={{backgroundImage:`radial-gradient(ellipse at 100% 35%,#17121ae8,transparent 65%),linear-gradient(transparent,#17121add),url("${backgroundSrc}")`}} aria-label="戦闘リザルト">
  ...
  <div className="recorded-result-body" tabIndex={0} aria-label="戦績と報酬">
   {mvp&&unit?<><div className="recorded-mvp"><div className="recorded-mvp-art"><img src={characterArt(unit,'full')??unit.image} alt={unit.name}/></div><div className="recorded-mvp-name"><small>MVP</small><h3>{unit.name}</h3><strong><span data-mvp-score={mvp.score.total}>{score}</span> <small>PT</small></strong></div></div><StatGrid className="recorded-result-stats" items={[{label:"与ダメージ",value:mvp.raw.damage},{label:"撃破",value:mvp.raw.kills,unit:"体"},{label:"回復",value:mvp.raw.heal}]}/></>:<p className="recorded-result-notice">{analysis.unavailable??'MVP対象の武将がいません。'}</p>}
   <details><summary>戦績・MVP詳細</summary><div className="recorded-result-panel">{mvp&&<><h3>MVPスコア内訳</h3><dl>{([['damage','与ダメージ',40],['kills','撃破',20],['heal','回復',20],['shield','シールド',15],['survival','生存',5]] as const).map(([key,label,max])=><InfoRow key={key} label={label} value={`${mvp.score[key]} / ${max}`}/>)}</dl></>}<h3>部隊の戦果</h3>{result.analysis.map(a=><p key={a.id}>{a.name}<br/>与ダメージ {a.damage.toLocaleString()} · 回復 {a.healing.toLocaleString()}</p>)}<details><summary>戦闘ログ</summary>{result.frames.map(f=><p key={f.index}>第{f.wave}派 · {f.text.replace(/Wave\s*(\d+)/gi,'第$1派')}</p>)}</details></div></details>
