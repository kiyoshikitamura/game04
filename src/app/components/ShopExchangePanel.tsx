...
const labels: Record<ShopExchangeId, string> = { energy_drink: "回復薬 ×1", cash_3000: "銭 ×3,000", cash_5000: "銭 ×5,000", cash_10000: "銭 ×10,000", cash_30000: "銭 ×30,000", cash_50000: "銭 ×50,000", raid_unlock: "領土侵攻解放素材 ×1", soul_generic: "同一レアリティ魂 → 汎用魂" };
...
export default function ShopExchangePanel({ state, onExchange }: Props) {
  ...
  const canExchange = Boolean(option && state.diamonds >= gemCost && (selected !== "soul_generic" || (soulAmount >= 10 && soulAmount % 2 === 0 && ownedSoul >= soulAmount)));
  const submit = async () => {
    if (!selected || !canExchange || busy) return;
  ...
  const card = (id: ShopExchangeId) => { const current = SHOP_EXCHANGE_OPTIONS.find(entry => entry.id === id)!; return <div key={id} className="shop-exchange-card"><div><strong>{labels[id]}</strong><p>{id === "soul_generic" ? "同一レアリティのキャラ魂2個につき汎用魂1個" : `${current.cost.toLocaleString("ja-JP")}輝石`}</p></div><OutlawButton variant="primary" disabled={busy} onClick={() => { setSelected(id); setQuantity(1); setError(""); }}>{id === "soul_generic" ? "交換する" : "交換"}</OutlawButton></div>; };
  return <section className="shop-section" aria-label="交換所">
    <div className="shop-section-title">交換所</div><p className="shop-card-desc">正式Masterに定義された交換のみ利用できます。無償輝石を先に使用します。</p>
    ...
    <div className="shop-exchange-soul">{card("soul_generic")}<p className="shop-card-desc">未所持キャラの魂も交換できます。逆交換はできません。</p></div>
    {selected && <CanonicalDialog title={`${labels[selected]}を交換`} onClose={() => !busy && setSelected(null)} actions={[{ label: busy ? "処理中" : "交換する", disabled: !canExchange || busy, onClick: submit }, { label: "閉じる", onClick: () => setSelected(null) }]}>
      {selected === "soul_generic" ? <><label>キャラ魂<select value={characterId} onChange={event => setCharacterId(event.target.value)}>{CHARACTER_MASTERS.map(character => <option key={character.id} value={character.id}>{character.name}（{character.rarity}）</option>)}</select></label><label>交換する数<input type="number" min={10} step={2} value={soulAmount} onChange={event => setSoulAmount(Number(event.target.value))} /></label><p>所持 {ownedSoul} → 汎用魂 {Math.floor(soulAmount / 2)}個</p></> : <>{selected === "energy_drink" && <label>交換数<input type="number" min={1} max={10} value={quantity} onChange={event => setQuantity(Math.min(10, Math.max(1, Number(event.target.value))))} /></label>}<p>必要：輝石 {gemCost.toLocaleString("ja-JP")} ／ 所持：輝石 {state.diamonds.toLocaleString("ja-JP")}</p></>}
