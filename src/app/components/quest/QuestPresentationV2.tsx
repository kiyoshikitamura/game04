...
import { GAME04_CHAPTERS, game04WorldText } from "@/theme/world";
...
const TOWNS = [
  ["shinjuku", "尾張"], ["shibuya", "美濃"], ["ikebukuro", "近江"], ["roppongi", "京洛"], ["akihabara", "甲斐"], ["kawasaki", "越後"], ["yokohama", "天下分け目"],
] as const;
...
export default function QuestPresentationV2() {
  ...
  const enemyMemberIds: string[] = Array.isArray(activeCourse?.enemy_members) && activeCourse.enemy_members.length
    ? activeCourse.enemy_members.map((entry: any) => entry.characterId || entry.character_id)
    : CANONICAL_QUEST_ENEMY_POOLS.entries.filter((entry) => entry.areaId === String(activeCourse?.town_id || "").toUpperCase() && entry.difficulty === activeCourse?.level_type).map((entry) => entry.characterId);
  const enemyCandidates = enemyMemberIds.map(id => CHARACTERS_MASTER.find(master => master.id === id)).filter(Boolean);
  ...
  return <HubPage className="patrol-container quest-v2-shell" title="クエスト" hideVisualHeader>
    <QuestTownStory townId={selectionVisible && ((selectionStep !== "DESTINATION" && activeCourse?.is_unlocked !== false) || selectedTownHasUpperClear) ? game.selectedTown : null} phase={selectedTownHasUpperClear ? "CLEAR" : "START"} stage={activeCourse?.level_type || "EASY"} completedStages={(game.patrolCourses || []).filter((course: any) => course.town_id === game.selectedTown && course.is_first_cleared).map((course: any) => course.level_type)} />
    <div ref={contentRef} className="quest-v2-content" style={{ "--quest-state-background": `url(${stateBgImage})` } as React.CSSProperties}>
      ...
      {!selectionVisible && !selectedPatrol && <section className="quest-v2-overview" aria-label="探索状況">
        ...
        {activePatrols.map((patrol: any) => {
          ...
          return <button className="quest-v2-dispatch-card" data-state={state} key={patrol.id} onClick={() => openPatrol(patrol.id)}>
            ...
            <span className="quest-v2-dispatch-description"><strong>{character?.jpName || "キャラクター"}</strong><span>{game04WorldText(course?.name) || "クエスト"} / {difficulty(course?.level_type || "")}</span><b>{label}{state === "WAITING" ? `・残り ${clock(patrol.secondsLeft)}` : ""}</b>{isCharacterHometown(character?.homeTown, course?.town_id) && <em className="quest-v2-bonus">地元一致</em>}</span>
      ...
      {activeCourse && selectionStep !== "DESTINATION" && <section className="quest-v2-brief">
        <header><strong>{game04WorldText(activeCourse.name)} / {difficulty(activeCourse.level_type)}</strong>{selectionStep === "CHARACTER" && <button className="quest-v2-back" onClick={() => setSelectionStep("REVIEW")}>戻る</button>}</header>
        {selectionStep === "REVIEW" && <>
      ...
      {!selectionVisible && selectedPatrol && <div className="quest-v2-progress-list">{[selectedPatrol].map((patrol: any) => {
        ...
        const npc = canonicalEnemyMembers.length > 0 ? {
          ...
          npc_name: game04WorldText(course?.name) || "クエスト",
        ...
        if (battleRequired) return <section className="tutorial-quest-wire quest-v2-state-surface quest-v2-battle-ready" data-quest-state="BATTLE_READY" key={patrol.id}>
          <div className={`tutorial-wire-encounter ${unresolvedBattle ? "is-ready" : ""}`}>
            ...
            <header className="quest-v2-battle-ready-identity"><small>{patrolTownName} / {difficulty(course?.level_type || "")}</small><h2>ボスに挑戦</h2><strong>{game04WorldText(course?.name) || "クエスト"}</strong></header>
