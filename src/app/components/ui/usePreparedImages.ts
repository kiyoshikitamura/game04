...
function decodeElement(img:HTMLImageElement):Promise<void>{
 ...
 return new Promise((resolve,reject)=>{
  const timer=setTimeout(()=>reject(new Error('timeout')),PRESENTATION.assetTimeoutMs);
  if(img.complete&&!img.naturalWidth)img.src=img.src;
  void img.decode().then(()=>{clearTimeout(timer);const record=elements.get(img);if(record)record.ready=source;resolve();},error=>{clearTimeout(timer);reject(error);});
 });
}
...
function prepare(url: string): Promise<void> {
  ...
  const promise = new Promise<void>((resolve,reject) => {
    ...
    const finish = (error?: Error) => { clearTimeout(timer); image.onload=null;image.onerror=null; if(error)reject(error);else{decoded.add(url);resolve();} };
...
function sources(root: HTMLElement) {
  ...
  for(const node of [root,...Array.from(root.querySelectorAll<HTMLElement>('*'))]) {
    if(node.closest('[hidden], [data-image-gate-ignore]') || !node.getClientRects().length || getComputedStyle(node).display==='none')continue;
...
export function usePreparedImages(ref: RefObject<HTMLElement | null>, screenKey?: unknown) {
  useLayoutEffect(()=>{
    ...
    const check=(force=false)=>{
      ...
      const images=Array.from(root.querySelectorAll('img')).filter(img=>img.getClientRects().length&&!!img.getAttribute('src'));
    ...
    observer.observe(root,{childList:true,subtree:true,attributes:true,attributeFilter:['src','srcset','style','hidden','open']});
    return()=>{disposed=true;generation++;cancelAnimationFrame(paint);transition.end();observer.disconnect();release?.();root.classList.remove('g4-preparing');root.setAttribute('data-images-ready','true');root.removeAttribute('aria-busy');};
  },[ref,screenKey]);
}
