...
function Card({ entry, now, resolve, onOpenRoom, disabled, rescue }: { entry: RaidTopEntry; now: number | null; resolve: Props['resolve']; onOpenRoom: Props['onOpenRoom']; disabled?: boolean; rescue?: boolean }) {
  ...
  return <article className="raid-approved-ui__card"><div className="raid-approved-ui__card-art">{enemy && <><img src={resolve(enemy.backgroundUrl)} alt="" /><img src={resolve(enemy.leaderImageUrl)} alt="" /></>}</div><div className="raid-approved-ui__card-copy"><div className="raid-approved-ui__badges"><span>{rescue ? '救援' : getRaidDifficultyLabel(entry.room.difficultyId)}</span><span>{rescue ? '参加可能' : '参加中'}</span></div><h3>{enemy?.bossName ?? '敵情報未確認'} <small>Lv.1</small></h3><p className="raid-approved-ui__attribute">{icon('medal')} {rescue ? '救援要請' : 'レイド進行中'}</p><Owner entry={entry} resolve={resolve}/><Hp entry={entry} now={now} compact={rescue}/><OutlawButton loadingLabel="" variant="primary" disabled={disabled} onClick={() => onOpenRoom(entry.room.roomId, entry.rescue.status === 'available' ? entry.rescue.value.rescueId : undefined)}>{rescue ? '救援に向かう' : '続きへ'} ›</OutlawButton></div></article>;
}
