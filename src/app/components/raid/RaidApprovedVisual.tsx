...
export function RaidApprovedDetailVisual({ data, resolve = value => value, compact = false, actions, contribution, challenge }: { data: RaidApprovedDetailData; resolve?: (url: string) => string; compact?: boolean; actions: ReactNode; contribution: ReactNode; challenge: ReactNode }) {
  return <div className="raid-approved-detail">
    {compact && <div className="raid-approved-detail__compact"><strong>{data.bossName} <small>Lv.1</small></strong><span>{data.ownerName}</span><b>{data.hpValueLabel}</b></div>}
    ...
    <section className="raid-approved-detail__owner">{data.ownerImageUrl ? <img src={resolve(data.ownerImageUrl)} alt={data.ownerName} /> : <span />}<div><strong>開催者　{data.ownerName}</strong><small>{data.guildLabel}</small></div></section>
    <section className="raid-approved-detail__battle"><div className="raid-approved-detail__hp"><div><span style={{ width: `${data.hpPercent ?? 0}%` }} /></div><strong>{data.hpValueLabel}</strong></div><div className="raid-approved-detail__facts"><span><RaidApprovedIcon name="clock" />{data.remainingLabel}<small>{data.expiryLabel}</small></span><span><RaidApprovedIcon name="people" />{data.participantLabel}<small>登録参加者</small></span></div>{data.faces && <div className="raid-approved-detail__faces">{data.faces.map(face => <img key={face.id} src={resolve(face.url)} alt={face.name} />)}</div>}</section>
...
export function RaidApprovedContribution({ damageLabel, damageValue, battlesLabel, eligibilityLabel, completed = 0 }: { damageLabel: string; damageValue: string; battlesLabel: string; eligibilityLabel: string; completed?: number }) {
  return <section className="raid-approved-detail__contribution"><h2>あなたの貢献</h2><div className="raid-approved-detail__damage"><RaidApprovedIcon name="swords" /><span>貢献ダメージ<strong>{damageValue}</strong></span><b>{damageLabel}</b></div><div className="raid-approved-detail__stats"><span><RaidApprovedIcon name="armor" />戦闘回数　{battlesLabel}</span><span><RaidApprovedIcon name="victory" />勝利　{completed}勝</span></div><div className="raid-approved-detail__eligibility"><span>{eligibilityLabel}</span><div>{[1, 2, 3].map(step => <span className={completed >= step ? 'is-done' : ''} key={step}><RaidApprovedIcon name="medal" /></span>)}</div></div></section>;
}
