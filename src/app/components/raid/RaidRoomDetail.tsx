...
export default function RaidRoomDetail({ room, briefing, display, participants, currentUserId, now, busy, onParticipants, onRewards, onEnemyInfo, action, rescue }: RaidRoomDetailProps) {
  ...
  const attributeLabels: Record<string, string> = { EVIL: "悪", ORDER: "秩序", JUSTICE: "正義", CHAOS: "混沌", UNKNOWN: "属性未取得" };
  const roomLevel = room.raidLevel?.status === "available" ? room.raidLevel.value : null;
  const roomCapacity = room.capacity?.status === "available" ? room.capacity.value : null;
  const roomAttribute = room.attribute?.status === "available" ? room.attribute.value : null;
  const detailData = { areaLabel: enemy?.areaName ?? "エリア未接続", bossName: enemy?.bossName ?? (brief?.raidVariantId ? "GAME04敵Master未接続" : "敵情報未取得"), backgroundUrl: enemy?.backgroundUrl, characterUrl: enemy?.leaderImageUrl, ownerName: owner?.name ?? "未確認", ownerImageUrl: ownerUrl, guildLabel: guild, hpPercent: percent, hpValueLabel: hp ? `${number(hp.current)} / ${number(hp.max)}` : "HP未確認", remainingLabel: lifecycle.remainingLabel, expiryLabel: Number.isFinite(expires) ? `期限 ${new Date(expires).toLocaleString("ja-JP", { timeZone: "Asia/Tokyo", month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })} JST` : "期限未確認", participantLabel: room.participantCount.status === "available" && roomCapacity !== null ? `${number(room.participantCount.value)}/${roomCapacity}人` : "未確認", faces: faceImages, levelLabel: roomLevel === null ? "Lv.未取得" : `Lv.${roomLevel}`, attributeLabel: attributeLabels[roomAttribute ?? enemy?.attribute ?? "UNKNOWN"] ?? "属性未取得", capacityLabel: roomCapacity === null ? "定員未取得" : `定員${roomCapacity}人` };
  ...
  const victoryLabel = me?.victoryCount?.status === "available" ? `${number(me.victoryCount.value)}勝` : "未取得";
  const recentLabel = me?.recentState?.status === "available" ? (me.recentState.value === "victory" ? "勝利" : me.recentState.value === "defeat" ? "敗北" : "未確定") : "未取得";
  const eligibilityLabel = me?.rewardEligibility?.status === "eligible" ? "報酬資格　対象" : me?.rewardEligibility?.status === "ineligible" ? "報酬資格　対象外" : "報酬資格　未取得";
  const progressLabel = me?.participationProgress?.status === "available" ? `参加時の共通進行　${number(me.participationProgress.value)}戦` : "参加時の共通進行　未取得";
  const completed = me?.participationProgress?.status === "available" ? me.participationProgress.value : 0;
  const contribution = isJoined ? <RaidApprovedContribution damageLabel="累積" damageValue={me?.appliedDamage.status === "available" ? number(me.appliedDamage.value) : "未確認"} battlesLabel={me?.finalizedBattles.status === "available" ? `${number(me.finalizedBattles.value)}戦` : "未確認"} victoryLabel={victoryLabel} recentLabel={recentLabel} eligibilityLabel={eligibilityLabel} progressLabel={progressLabel} completed={completed} /> : null;
