  ...
  return (
    <FullScreenPanel title="設定 / プロフィール" onClose={() => { if (!game.profileLoading) close(); }}>
      <div className="settings-panel-container-inner">
        ...
        {!redesign && <SeasonHonors ownerId={game.session?.user?.id} scope="USER" editable onEquipped={(slot, id) => { if (slot === "PROFILE_TITLE") game.setTitleEquipped(id); }} />}
        {!redesign && isPreOpen && <EditableSettingSection title="ホーム背景" helper="背景を選んで保存できます" editing={homeEditing} pending={game.profileLoading} onEdit={() => setHomeEditing(true)} summary={<p>{game.selectedBgMode === "auto" ? "夕桜の城下町（標準）" : availableBackgrounds.find((item) => item.id === game.selectedBgMode)?.name || "未設定"}</p>}>
          <ChoiceGroup label="背景" value={backgroundDraft} disabled={game.profileLoading} onChange={setBackgroundDraft} options={[{ value: "auto", label: "夕桜の城下町（標準）" }, ...availableBackgrounds.map((item) => ({ value: item.id, label: item.name }))]} />
        ...
        {!redesign && !isPreOpen && <EditableSettingSection title="ホーム演出" helper="所持中の装飾を選択できます" editing={homeEditing} pending={game.profileLoading} onEdit={() => setHomeEditing(true)} summary={<dl className="settings-summary"><div><dt>背景</dt><dd>{game.selectedBgMode === "auto" ? "夕桜の城下町（標準）" : availableBackgrounds.find((item) => item.id === game.selectedBgMode)?.name || "未設定"}</dd></div><div><dt>前景</dt><dd>{availableFrontEffects.find((item) => item.id === game.equippedFrontEffect)?.name || "なし"}</dd></div><div><dt>内装</dt><dd>{availableInteriors.find((item) => item.id === game.interiorItem)?.name || "なし"}</dd></div></dl>}>
          <ChoiceGroup label="背景" value={backgroundDraft} disabled={game.profileLoading} onChange={setBackgroundDraft} options={[{ value: "auto", label: "夕桜の城下町（標準）" }, ...availableBackgrounds.filter((item) => item.id !== "auto").map((item) => ({ value: item.id, label: item.name }))]} />
        ...
        {!redesign && canProvisionQa && <section className="settings-section"><h4 className="settings-section-title">QAテストデータ</h4><p className="settings-help-text">このアカウントのテスト用所持データを再投入します。</p><OutlawButton variant="secondary" fullWidth disabled={qaLoading} onClick={() => void provisionQa()}>{qaLoading ? "投入中..." : "テストデータを投入"}</OutlawButton></section>}
