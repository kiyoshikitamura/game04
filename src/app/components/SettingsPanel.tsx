  ...
  return (
    <FullScreenPanel title="設定 / プロフィール" onClose={() => { if (!game.profileLoading) close(); }}>
      <div className="settings-panel-container-inner">
        ...
        {isPreOpen && <EditableSettingSection title="ホーム背景" helper="背景を選んで保存できます" editing={homeEditing} pending={game.profileLoading} onEdit={() => setHomeEditing(true)} summary={<p>{game.selectedBgMode === "auto" ? "夕桜の城下町（標準）" : availableBackgrounds.find((item) => item.id === game.selectedBgMode)?.name || "未設定"}</p>}>
        ...
        {!isPreOpen && <EditableSettingSection title="ホーム演出" helper="所持中の装飾を選択できます" editing={homeEditing} pending={game.profileLoading} onEdit={() => setHomeEditing(true)} summary={<dl className="settings-summary"><div><dt>背景</dt><dd>{game.selectedBgMode === "auto" ? "夕桜の城下町（標準）" : availableBackgrounds.find((item) => item.id === game.selectedBgMode)?.name || "未設定"}</dd></div><div><dt>前景</dt><dd>{availableFrontEffects.find((item) => item.id === game.equippedFrontEffect)?.name || "なし"}</dd></div><div><dt>内装</dt><dd>{availableInteriors.find((item) => item.id === game.interiorItem)?.name || "なし"}</dd></div></dl>}>
