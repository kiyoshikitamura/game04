...
export function previewReplacementAllowed(env: Record<string, string | undefined>): boolean {
  return env.GOOGLE_ACCOUNT_REPLACEMENT_ENABLED === 'true' && env.VERCEL_ENV !== 'production' && isValidSupabaseUrl(env.NEXT_PUBLIC_SUPABASE_URL ?? '', env.NEXT_PUBLIC_APP_ENV ?? 'development');
}
