...
const OUT='docs/verification/stage68-duration-20260927',folder=process.argv[2]??'pressure-aoe',extended=process.argv.includes('--extend'),ids=process.argv.slice(3).filter(x=>!x.startsWith('--')),sig=c=>hash(c.input.party);
for(const row of rows){if(ids.length&&!ids.includes(row.stage))continue;const p=`${OUT}/${folder}/${row.stage}.json.gz`;if(!fs.existsSync(p))continue;const source=gz(p),v=source.variants.find(v=>v.name===source.selected);if(!v||v.records.some(c=>!c.validation||metrics(c.validation).actionLimit))continue;
 const path=`${OUT}/fill-${folder}/${row.stage}.json.gz`,d=fs.existsSync(path)?gz(path):{stage:row.stage,source:p,selected:source.selected,stageProposal:v.stage,records:structuredClone(v.records),screened:0,complete:false};if(d.complete&&(!extended||d.passed||d.guardFailure))continue;d.complete=false;
 const old=stageResults(row),extra=[];for(const f of ['fill-pressure-aoe','fill-pressure-combined','fill-pressure-refined','fill-pressure-refined2','fill-pressure-refined3']){const p=`${OUT}/${f}/${row.stage}.json.gz`;if(fs.existsSync(p)&&p!==path)extra.push(...gz(p).records.filter(c=>c.validation&&metrics(c.validation).actionLimit>0));}const guard=[...old.assessment.stalled,...extra,...old.assessment.dependency.map(d=>d.without).filter(Boolean)];
 for(const c of guard){if(d.records.some(r=>sig(r)===sig(c)))continue;const input=inputFor(v.stage,{party:c.input.party}),r={name:c.name,state:c.state,family:c.family,input,inputHash:hash(input),validation:run(input,712001,200,true),purpose:'old-stall-and-skill-independence-guard'};d.records.push(r);write(path,d);if(metrics(r.validation).actionLimit){d.guardFailure=r.name;break;}}
 if(!d.guardFailure){const seen=new Set(d.records.map(sig));let q=assess(d.records.filter(c=>c.validation));
  while(q.missing.length&&d.screened<(extended?180:80)){const band=q.missing[d.screened%q.missing.length],bases=d.records.filter(c=>c.validation).sort((a,b)=>Math.abs(a.validation.rate-targets[band])-Math.abs(b.validation.rate-targets[band]));let next;
   ...
   if(!next)break;next.screen=run(next.input,713001,24,true);if(q.missing.some(i=>inBand(next.screen.rate,i))){next.validation=run(next.input,712001,200,true);console.log(JSON.stringify({stage:row.stage,folder,name:next.name,wins:next.validation.wins,stall:metrics(next.validation).actionLimit}));}
