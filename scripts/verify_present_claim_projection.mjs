...
function fixture(mode='success',legacy=true) {
  ...
  const supabase={async rpc(){calls++;await gate;if(mode==='reject')return {error:Error('network')};rows=rows.map(p=>({...p,status:'CLAIMED'}));if(mode==='lost')throw Error('network');return {data:{status:'success',claimed_count:1}};},from(table){if(!legacy&&table==='user_equipments')throw Error('GAME04 must not query legacy equipment');const q={select(){return q;},eq(){return q;},order(){return q;},single(){return q;},then(resolve,reject){const data=table==='users'?{cash:10,neon_diamonds:30}:table==='presents'?rows:table==='user_items'?[{item_id:'ENERGY_DRINK',quantity:5}]:[{id:'gear'}];return Promise.resolve(mode==='readfail'?{error:Error('read failed')}:{data}).then(resolve,reject);}};return q;}};
  const exports={};vm.runInNewContext(code,{exports,console,require(name){if(name==='react')return react;if(name.includes('redesignRewardSync'))return {notifyRedesignRewardChange:()=>{}};if(name.endsWith('/supabase'))return {supabase};if(name.includes('useImmediateActionLock'))return {useImmediateActionLock(){const lock=react.useRef(false);return {isLocked:lock.current,beginAction(){if(lock.current)return false;lock.current=true;return true;},endAction(){lock.current=false;}};}};if(name.includes('actionPerformance'))return {beginActionPerformance:()=>({mark(){},markVisualReady(){}})};if(name.includes('inventoryProjection'))return {buildInventoryQuantityProjection:()=>({ENERGY_DRINK:5})};return {canonicalItemName:x=>x};}});
  const render=()=>{cursor=0;return exports.useInventory({user:{id:owner}},0,v=>cash=v,0,v=>diamonds=v,0,()=>{},()=>{},async()=>bootstrap++,v=>dialog=v,undefined,legacy?v=>equipment=v:undefined);};
...
for(const bulk of [false,true]) {
  ...
  const current=fixture('success',false);await run(current);assert.equal(current.dialog.rewards[0].quantity,10);assert.equal(current.equipment.length,0);
