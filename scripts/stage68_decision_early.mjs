...
for(const id of ids){
 const path=`${out}/${id}-early.json.gz`,checkpoint=fs.existsSync(path)?JSON.parse(zlib.gunzipSync(fs.readFileSync(path))):null;if(checkpoint?.validation){console.log(id,'checkpoint already saved');continue;}
 ...
 if(checkpoint)assert.equal(checkpoint.inputHash,inputHash,'checkpoint input changed');
 const result=checkpoint??{stage:id,name,core,state,loadout:loadout(state),costs:costs(state),input,inputHash,enemyUnchanged:true,supplyChanged:false,newTrials:0,status:'SCREEN_ONLY'};
 if(prior){result.validation=prior.validation;result.reused={path:`../stage68-five-tier-20260926/${id}-validated.json.gz`,name:prior.name};result.status='REUSED';}
  ...
  if(!result.screen){result.screen=run(input,210001,20,true);result.newTrials=20;save(path,result);}
