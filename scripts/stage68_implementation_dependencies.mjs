...
for(const id of (terminal?['3-5']:finisher?['10-5']:micro?['10-6']:['9-6','10-6','10-7','10-8'])){
 const path=`${OUT}/${terminal?'dependencies-terminal':finisher?'dependencies-finisher':micro?'dependencies-micro':'dependencies'}/${id}.json.gz`,t=gz(`${OUT}/${terminal?'stall-terminal':finisher?'stall-finisher':micro?'stall-micro':'stalls'}/${id}.json.gz`),v=t.variants.find(v=>micro?v.factor===t.selectedFactor:v.name===t.selected),cs=candidatesFor(rows.find(r=>r.stage===id)).cs.filter(c=>c.asset.covered),search=gz(`${OUT}/${terminal?'search-terminal':finisher?'search-finisher':micro?'search-micro':'search-stall'}/${id}.json.gz`).records.filter(c=>c.validation),known=[...v.records,...search];
