...
const dir = 'docs/verification/device-debug/evidence-b03';
...
(async () => {
  ...
  try {
    ...
    assert(Math.abs(await battle.evaluate(e=>Number(e.style.getPropertyValue('--battle-speed')))-1)<.001);
    for (const width of [375,390,430]) {
      await page.setViewportSize({width,height:664});
      const skills=page.locator('[class*="member"][data-unit-id]').first().locator('[class*="skills"] button');
      assert.equal(await skills.count(),3);
      const boxes=await skills.evaluateAll(es=>es.map(e=>{const r=e.getBoundingClientRect();return {x:r.x,right:r.right,y:r.y,bottom:r.bottom,width:r.width,height:r.height};}));
      for(let i=0;i<3;i++){assert(boxes[i].x>=0&&boxes[i].right<=width);if(i)assert(boxes[i-1].right<=boxes[i].x);await skills.nth(i).click();assert(await page.locator('dialog[open]').count()>0);await page.locator('dialog[open] button').filter({hasText:'閉じる'}).first().click();}
    }
    ...
    for(const text of ['通常攻撃','検証用 攻撃','検証用の敵 −','豊臣秀吉 −','BURST発動']) {
      ...
      if(text==='検証用 攻撃') assert.equal(await page.locator('[class*="skillFeedback"]').count(),0);
    }
