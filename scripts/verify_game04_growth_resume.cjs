...
const specific=m.AWAKENING_SOULS[rarity][0];const exchangeAmount=Math.max(10,specific*2);s.souls[id]=exchangeAmount+1;
let exchanged=g.applyGrowthAction(s,'soul_exchange',{characterId:id,amount:exchangeAmount});assert.equal(exchanged.souls[id],1);assert.equal(exchanged.growthInventory.genericSouls[rarity],exchangeAmount/2);
